...
function cancelar() {
  document.forms[0].action="formEntidadeContatoSimplificado.jsp";
  document.forms[0].submit();
}

function voltar(cdEntidade) {
  document.forms[0].action = 'listarEntidadeContatoSimplificado.jsp?acao=voltar&cdEntidade='+cdEntidade;
	document.forms[0].submit();
}  
...
<%
//variáveis capturadas da página listarEntidadeContatoSimplificado.jsp
...
<form method="post" action="listarEntidadeContatoSimplificado.jsp?cdEntidade=<%=cdEntidade%>&acao=<%=acao%>&idEntidadeContato=<%=idEntidadeContato%>&acaoAnterior=<%=acaoAnterior%>">
