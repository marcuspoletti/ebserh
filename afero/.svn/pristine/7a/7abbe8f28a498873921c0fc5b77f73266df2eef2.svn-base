...
if (idPedidoSaida == null || idPedidoSaida.equals("0")){
	...
	if(acao.equalsIgnoreCase("atu")){
		if(!cdEntidade.equals("0")){
			daoPedidoSaida.atualizarClientes(Integer.parseInt(idPedidoSaida), Integer.parseInt(cdEntidade), usuario);
			cdEntidade = "".valueOf(cdEntidade);
		}else{
	...
	idEntrega = "".valueOf(pedidoSaida.getIdEntrega());
         idColaborador = pedidoSaida.getIdColaborador();
...
<h1 class="cabecalho_pagina">Editar Pedido</h1>
  ...
  <tr>
    ...
    <tr>
      <th class="label" style="height: 22px">Cliente*</th>
         <td style="height: 15px"><input disabled type="text" name="dsEntidade" <%if (dsEntidade != null) { %>value="<%=dsEntidade%>"<% }%>  size="60" maxlength="60"><a href="editarConsultarClientes.jsp?acao=<%="atu"%>&idPedidoSaida=<%=idPedidoSaida%>&idLoja=<%=idLoja%>&idEntrega=<%=idEntrega%>&idColaborador=<%=idColaborador%>"><img border = "0" src="../images/pesquisa.gif"></a></td>       		
  </tr>
    <tr>
