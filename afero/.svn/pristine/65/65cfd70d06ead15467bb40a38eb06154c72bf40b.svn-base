...
function localizar() {
  document.forms[0].action="atualizarEstoque.jsp";
...
function recarregar(acao) {
  document.forms[0].action='atualizarEstoque.jsp?acao='+acao;
  document.forms[0].submit();
}
...
//clausula +=" AND e.idLoja='"+idLoja+"'";
List list;
  	
dao = new ProdutoDAO(conn);
clausula = clausula+" ORDER BY p.dsProduto ";
list = dao.listarProdutoPrecoLoja(clausula, idLoja);
...
<form action="listarAtualizacaoEstoque.jsp" method="post">
...
%>
  <tr>
    ...
    <td class = "grid"><center><%=prod.getNmProduto()%> / <%=prod.getDsProduto() %></center></td>
