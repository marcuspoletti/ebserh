...
<%@ page import="afero.model.SaidaProduto" %>
...
if (dataFim == null) {
    ...
    clausula = " WHERE ps.dtPed BETWEEN '"+converte.DMYToYMDI(dataIni)+" 00:00:00' AND '"+converte.DMYToYMDI(dataFim)+" 23:59:59'";
...
list = dao.listarRelatorioSaidaProduto(clausula);
%>
<h1 class="cabecalho_pagina">Listagem de Saídas do Estoque</h1>
...
<form method="post" action="relSaidaProduto.jsp">
...
<table border="0" width="100%">
    <tr>
      ...
      <th class="grid" width="2%"><center>Dsc. Prod.</center></th>
...
for (Iterator it = list.iterator(); it.hasNext();) {
	SaidaProduto saidaProd = (SaidaProduto) it.next();
	if(saidaProd.getCdProduto() != null){
		codigoProduto = saidaProd.getCdProduto(); 
	}else{
		codigoProduto = "Não esta cadastro";
	}
	cont++;
	totalCusto +=saidaProd.getCusto();
	totalPrUnit += saidaProd.getPrUnit();
%>
  
   <tr>
      <td class = "grid" width="2%"><center><%=cont%></center></td>
      <td class = "grid" width="5%"><center><%=codigoProduto%></center></td>
      <td class = "grid" width="15%"><center><%=saidaProd.getDsProduto()%></center></td>
       <td class = "grid" width="5%"><center><%=saidaProd.getNumPedido()%></center></td>
       <td class = "grid" width="5%"><center><%=ConverteDate.dateToString(saidaProd.getDtPed())%></center></td>
      <td class = "grid" width="5%"><center><%=Utilitaria.formatarNumero(saidaProd.getQuant(),0)%></center></td>
      <td class = "grid" width="5%"><center><%=Utilitaria.formatarNumero(saidaProd.getPrUnit(),2)%></center></td>
      <td class = "grid" width="5%"><center><%=Utilitaria.formatarNumero(saidaProd.getCusto(),2)%></center></td>
</tr>
<%
}
%>    
  ...
  <td class = "grid" width="5%"><center></center></td>
  <td class = "grid" width="5%"><font color="red"><center><%=Utilitaria.formatarNumero(totalCusto,2)%></font></center></td>
  </tr>  
  <tr>
  <br>
