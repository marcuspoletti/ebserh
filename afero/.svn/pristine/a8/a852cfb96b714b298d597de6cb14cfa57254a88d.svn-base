...
clausula = clausula+" AND ps.tipoPedido = 'R' AND ps.operacao='S' AND ps.idLoja = "+Integer.parseInt(idLojaUsuario);
...
<table border="0" width="100%">
    <tr>
      ...
      <th class="grid" width="5%"><center>Núm. do Orc</center></th>
      <th class="grid" width="5%"><center>Núm. do Ped</center></th>
      <th class="grid" width="2%"><center>Dt do Ped</center></th>
      <th class="grid" width="5%"><center>Cliente</center></th>
      <th class="grid" width="5%"><center>Usuário</center></th>
      <th class="grid" width="5%"><center>Pessoa Resp.</center></th>
      ...
      <th class="grid" width="2%">Integração Financeiro</th> 
      <!--<th class="grid" width="2%">Gerar N.F.</th> --> 
    </tr>
<%
int cont = 0;
int gridSize = 20;
...
for ( Iterator it = list.iterator(); hasNext && cont < gridSize; hasNext = it.hasNext()) {
	...
	statusPag = dao.statusPag(pedidoSaida.getIdPedidoSaida());
	...
	if(statusPag != null){
		if(statusPag.equalsIgnoreCase("P")){
			dsStatusPag = "SIM";
		}else if(statusPag.equalsIgnoreCase("N")){
			dsStatusPag = "NÃO";
		}
	...
	if(dao.statusPag(pedidoSaida.getIdPedidoSaida()) != null){
		statusPagamento = dao.statusPag(pedidoSaida.getIdPedidoSaida());
	}else{
		statusPagamento = "N";
	}
...
%>
   ...
   <tr>
      ...
      <%if(numOrcamento == 0){ %>
      	<td class = "grid" width="5%"><font color="red"><center><%="PEDIDO"%></center></font></td>
      <%}else{ %>
      	 <td class = "grid" width="5%"><center><%=numOrcamento%></center></td>
      <%} %>
      <td class = "grid" width="5%"><center><a target="_blank" href="exibirPedido.jsp?idPedidoSaida=<%=pedSaida.getIdPedidoSaida()%>"><%=pedSaida.getIdPedidoSaida()%></a></center></td>
      <td class = "grid" width="5%"><center><%=ConverteDate.dateToString(pedSaida.getDtPed())%></center></td>
      <td class = "grid" width="20%"><center><%=entidade.getNome()%></center></td>
      <td class = "grid" width="10%"><center><%=pedSaida.getUsuario()%></center></td>
      <%if(pedSaida.getPessoaResponsavel() != null){ %>
      <td class = "grid" width="10%"><center><%=pedSaida.getPessoaResponsavel()%></center></td>
      <%}else{%>
      ...
      <%if((!pedSaida.getStatus().equalsIgnoreCase("A"))){%>
         <%if(statusPagamento.equalsIgnoreCase("N")) {%>
      	...
      	<%}else{ %>
      	     <td class = "grid" width="2%"><font color="red"><center><%="PAGO"%></center></font></td>
      	<%} %>
      ...
      <td class = "grid" width="3%"><font color="red"><center><%=dsStatusPag%></center></font></td>
    
       <%if(statusPagamento.equalsIgnoreCase("S")) {%>
           	<td class = "grid" width="2%"><center>-</center></td>
           
       <%}else{ %>
       		<td class = "grid" width="2%"><a target="_blank" href="../integracaofinanceiro/integracaoFinanceiro.jsp?idPedidoSaida=<%=pedSaida.getIdPedidoSaida()%>&acao=<%="listar"%>&cdEntidade=<%=entidade.getCdEntidade()%>&dsEntidade=<%=entidade.getNome()%>&idUsuario=<%=idUsuario%>"><img border="0"src="../images/calculadora.jpg"></a></img></td>
       <%} %>
    	<!-- <td class = "grid" width="2%"><a target="_blank" href="../notafiscal/gerarNotaFiscal.jsp?idPedidoSaida=<%=pedSaida.getIdPedidoSaida()%>"><img border="0"src="../images/edit.gif"></a></img></td> -->
    </tr>
<%
}
%>      
...
<%  if (rowNum != 0 || hasNext) { %>
        <center>
<%      if (rowNum != 0) { %>
<a class="link" href="consultarPedidoSaida.jsp?rowNum=<%=0%>&nomeLocalizar=<%=nomeLocalizar%>&statusLocalizar=<%=statusLocalizar%>">Primeira</a>&nbsp&nbsp
|&nbsp&nbsp<a class="link" href="consultarPedidoSaida.jsp?rowNum=<%=rowNum-gridSize%>&nomeLocalizar=<%=nomeLocalizar%>&statusLocalizar=<%=statusLocalizar%>">< Anterior</a>&nbsp&nbsp
<%      } else { %>
Primeira&nbsp&nbsp|&nbsp&nbsp< Anterior&nbsp
<%      } %>
<%      if (hasNext) { %>
|&nbsp&nbsp<a class="link" href="consultarPedidoSaida.jsp?rowNum=<%=rowNum+gridSize%>&nomeLocalizar=<%=nomeLocalizar%>&statusLocalizar=<%=statusLocalizar%>">Próxima ></a>&nbsp&nbsp
|&nbsp&nbsp<a class="link" href="consultarPedidoSaida.jsp?rowNum=<%=rowCount-gridSize%>&nomeLocalizar=<%=nomeLocalizar%>&statusLocalizar=<%=statusLocalizar%>">Última</a>
<%      } else { %>
