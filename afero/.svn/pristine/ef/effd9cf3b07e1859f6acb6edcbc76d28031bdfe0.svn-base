...
function cancelar() {
  document.forms[0].action="formEntidadeEndereco.jsp";
  document.forms[0].submit();
}

function voltar(cdEntidade) {
  document.forms[0].action = 'listarEntidadeEndereco.jsp?acao=voltar&cdEntidade='+cdEntidade;
	document.forms[0].submit();
}  
...
<%
//variáveis capturadas da página listarEntidadeEndereco.jsp
...
<form method="post" action="listarEntidadeEndereco.jsp?cdEntidade=<%=cdEntidade%>&acao=<%=acao%>&idEntidadeEndereco=<%=idEntidadeEndereco%>&acaoAnterior=<%=acaoAnterior%>">
