...
<script>
	function confirmaExcluir(cont, id) {
		if (confirm("Deseja mesmo apagar o registro [" + cont + "]?")) {
			location.href = 'listarDuplicataPagar.jsp?acao=exc&idDuplicata=' + id;
		}
	}
	...
	if (acao.equalsIgnoreCase("inc") || acao.equalsIgnoreCase("atu") || acao.equalsIgnoreCase("exc")) {
		...
		if (acao.equalsIgnoreCase("inc")) {
			...
			if (!daoExclusao.exclusaoIdDuplicata(Integer.parseInt(idDuplicata))) {
				...
				response.sendRedirect("listarDuplicataPagar.jsp?mensagem=Exclusão Inválida! Item já possui lançamentos.&acao=listar");
			}
		}
...
|<a class="button" href="formDuplicataPagar.jsp?acao=inc">Novo Cadastro</a>|<br>
...
<form action="listarDuplicataPagar.jsp">
    ...
    <tr>
  	...
  	<tr>
    	...
    	<td class="label_radio">
      	...
      	<input type="radio" class="radio" name="statusLocalizar" value="Q" <%=(statusLocalizar.equals("Q") ? "checked" : "")%>>Pago ao favorecido
...
%>
    <tr>
      ...
      <td class = "grid" width="5%"><center><a href="formDuplicataPagar.jsp?acao=atu&idDuplicata=<%=dup.getIdDuplicata()%>"><%=dup.getNrDoc()%> </a></center></td>
     	<td class = "grid"><center><%=conta.getNmConta()%></center></td>
     	<td class = "grid" width="10%"><center><a href="formDuplicataParcelaPagar.jsp?acao=atuP&idDuplicata=<%=dup.getIdDuplicata()%>&idDuplicataParcela=<%=dupParc.getIdDuplicataParcela()%>"><%=dup.getQuantParcelas()==1 ? "Única":"Parcela "+dupParc.getNrParcela()+"/"+dup.getQuantParcelas()%> </a></center></td>
     	...
     	%>
      	<td class = "grid" width="5%"><center><a href="formDuplicataParcelaPagamento.jsp?acao=atuP&idDuplicata=<%=dup.getIdDuplicata()%>&idDuplicataParcela=<%=dupParc.getIdDuplicataParcela()%>"><%=msgStatus%> </a></center></td>
      <%} else { %>
...
<p align="right">|<a class="button" href="formDuplicataPagar.jsp?acao=inc">Novo Cadastro</a>|</p>