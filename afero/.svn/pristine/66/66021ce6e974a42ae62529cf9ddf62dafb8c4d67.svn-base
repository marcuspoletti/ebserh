...
function cancelar() {
  document.forms[0].action="consultarProdutoDivisao.jsp";
  document.forms[0].submit();
}

function recarregar(acao) {
  document.forms[0].action='consultarProdutoDivisao.jsp?acao='+acao;
  document.forms[0].submit();
}
</script>
...
if(idDivisao!= 0){
	clausula = clausula +" AND idDivisao= "+idDivisao;
}
...
<h1 class="cabecalho_pagina">Consulta de Preços por Divisão</h1>

<hr>

<body onload="document.forms[0].elements[0].focus();" >
<form action="consultarProdutoDivisaoAdministrador.jsp?acao=atu">
...
<table colspan="2">
  ...
  <tr>
    ...
    <td class="label_radio">
      <input disabled type="radio" class="radio" name="status" value="" <%= (status.equals("")? "checked": "") %>>Todos
      <input type="radio" class="radio" name="status" value="A" <%= (status.equals("A")? "checked": "") %>>Ativo
      <input disabled type="radio" class="radio" name="status" value="I" <%= (status.equals("I")? "checked": "") %>>Inativo</td>
...
<%  if (rowNum != 0 || hasNext) { %>
        <center>
<%      if (rowNum != 0) { %>
<a class="link" href="consultarProdutoDivisaoAdministrador.jsp?rowNum=<%=0%>&nomeProduto=<%=nomeProduto%>&status=<%=status%>&idDivisao=<%=idDivisao%>&idLoja=<%=idLoja%>">Primeira</a>&nbsp&nbsp
|&nbsp&nbsp<a class="link" href="consultarProdutoDivisaoAdministrador.jsp?rowNum=<%=rowNum-gridSize%>&nomeProduto=<%=nomeProduto%>&status=<%=status%>&idDivisao=<%=idDivisao%>&idLoja=<%=idLoja%>">< Anterior</a>&nbsp&nbsp
<%      } else { %>
Primeira&nbsp&nbsp|&nbsp&nbsp< Anterior&nbsp
<%      } %>
<%      if (hasNext) { %>
|&nbsp&nbsp<a class="link" href="consultarProdutoDivisaoAdministrador.jsp?rowNum=<%=rowNum+gridSize%>&nomeProduto=<%=nomeProduto%>&status=<%=status%>&idDivisao=<%=idDivisao%>&idLoja=<%=idLoja%>">Próxima ></a>&nbsp&nbsp
|&nbsp&nbsp<a class="link" href="consultarProdutoDivisaoAdministrador.jsp?rowNum=<%=rowCount-gridSize%>&nomeProduto=<%=nomeProduto%>&status=<%=status%>&idDivisao=<%=idDivisao%>&idLoja=<%=idLoja%>">Última</a>
<%      } else { %>
