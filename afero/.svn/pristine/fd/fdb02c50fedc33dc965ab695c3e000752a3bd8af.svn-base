...
function localizar() {
  document.forms[0].action="consultarExtratoProduto.jsp";
...
function recarregar(acao) {
  document.forms[0].action='consultarExtratoProduto.jsp?acao='+acao;
  document.forms[0].submit();
}
...
<h1 class="cabecalho_pagina">Consulta de Extrato de Produto</h1>

<hr>

<body onload="document.forms[0].elements[0].focus();" >
<form action="consultarExtratoProduto.jsp?acao=listar">
...
%>
    <tr>
      ...
      <td class = "grid"><center><a target="_blank" href="extratoProduto.jsp?idProduto=<%=prod.getIdProduto()%>&idLoja=<%=idLoja%>"><%=prod.getDsProduto() %></a> </center></td>
...
<%  if (rowNum != 0 || hasNext) { %>
        <center>
<%      if (rowNum != 0) { %>
<a class="link" href="consultarExtratoProduto.jsp?rowNum=<%=0%>&nomeProduto=<%=nomeProduto%>&status=<%=status%>&idLoja=<%=idLoja%>">Primeira</a>&nbsp&nbsp
|&nbsp&nbsp<a class="link" href="consultarExtratoProduto.jsp?rowNum=<%=rowNum-gridSize%>&nomeProduto=<%=nomeProduto%>&status=<%=status%>&idLoja=<%=idLoja%>">< Anterior</a>&nbsp&nbsp
<%      } else { %>
Primeira&nbsp&nbsp|&nbsp&nbsp< Anterior&nbsp
<%      } %>
<%      if (hasNext) { %>
|&nbsp&nbsp<a class="link" href="consultarExtratoProduto.jsp?rowNum=<%=rowNum+gridSize%>&nomeProduto=<%=nomeProduto%>&status=<%=status%>&idLoja=<%=idLoja%>">Próxima ></a>&nbsp&nbsp
|&nbsp&nbsp<a class="link" href="consultarExtratoProduto.jsp?rowNum=<%=rowCount-gridSize%>&nomeProduto=<%=nomeProduto%>&status=<%=status%>&idLoja=<%=idLoja%>">Última</a>
<%      } else { %>
