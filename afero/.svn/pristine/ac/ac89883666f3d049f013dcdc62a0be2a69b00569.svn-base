...
%>
<h1 class="cabecalho_pagina">Editar Clientes</h1>
<hr>
<body onload="document.forms[0].elements[0].focus();" >
<form action="editarConsultarClientes.jsp">
...
%>
    <tr>
      ...
      <td class = "grid"><center><a class="link" target="principal" href="editarFormOrcamento.jsp?idOrcamento=<%=idOrcamento%>&cdEntidade=<%=cdEntidade%>&acao=<%="atu"%>&dsEntidade=<%=nomeEntidade%>&idLoja=<%=idLoja%>&idEntrega=<%=idEntrega%>&idColaborador=<%=idColaborador%>" onclick="javascript:window.close()"><%=nomeEntidade%></a></center></td>
...
<%  if (rowNum != 0 || hasNext) { %>
        <center>
<%      if (rowNum != 0) { %>
<a class="link" href="editarConsultarClientes.jsp?rowNum=<%=0%>&nomeLocalizar=<%=nomeLocalizar%>&statusLocalizar=<%=statusLocalizar%>&tpInsc=<%=tpInsc%>&cpfLocalizar=<%=cpfLocalizar%>&telefone=<%=telefone%>">Primeira</a>&nbsp&nbsp
|&nbsp&nbsp<a class="link" href="editarConsultarClientes.jsp?rowNum=<%=rowNum-gridSize%>&nomeLocalizar=<%=nomeLocalizar%>&statusLocalizar=<%=statusLocalizar%>&tpInsc=<%=tpInsc%>&cpfLocalizar=<%=cpfLocalizar%>&telefone=<%=telefone%>">< Anterior</a>&nbsp&nbsp
<%      } else { %>
Primeira&nbsp&nbsp|&nbsp&nbsp< Anterior&nbsp
<%      } %>
<%      if (hasNext) { %>
|&nbsp&nbsp<a class="link" href="editarConsultarClientes.jsp?rowNum=<%=rowNum+gridSize%>&nomeLocalizar=<%=nomeLocalizar%>&statusLocalizar=<%=statusLocalizar%>&tpInsc=<%=tpInsc%>&cpfLocalizar=<%=cpfLocalizar%>&telefone=<%=telefone%>">Próxima ></a>&nbsp&nbsp
|&nbsp&nbsp<a class="link" href="editarConsultarClientes.jsp?rowNum=<%=rowCount-gridSize%>&nomeLocalizar=<%=nomeLocalizar%>&statusLocalizar=<%=statusLocalizar%>&tpInsc=<%=tpInsc%>&cpfLocalizar=<%=cpfLocalizar%>&telefone=<%=telefone%>">Última</a>
<%      } else { %>
