...
function cancelar() {
  document.forms[0].action="formTipoColaborador.jsp";
  document.forms[0].submit();
}

function voltar() {
  document.forms[0].action = 'listarTipoColaborador.jsp?acao=voltar'
	document.forms[0].submit();
}  
</script>
</head>
...
if (acao.equalsIgnoreCase("atu")) {
	TipoColaboradorDAO dao = new TipoColaboradorDAO(conn);
	TipoColaborador tipoColaborador = dao.procurarTipoColaborador(Integer.parseInt(idTipoColaborador));
	descricao  = tipoColaborador.getDsColaborador();
	status = tipoColaborador.getStatus();
}
%>
<body onload="document.forms[0].elements[2].focus();" >
<h1 class="cabecalho_pagina">Cadastro Tipo Colaborador</h1>
<form method="post" action="listarTipoColaborador.jsp?acao=<%=acao%>&idTipoColaborador=<%=idTipoColaborador%>">
<input type="hidden" name="acao" value="<%=acao%>"/>
<input type="hidden" name="idTipoColaborador" value="<%=idTipoColaborador%>"/>
