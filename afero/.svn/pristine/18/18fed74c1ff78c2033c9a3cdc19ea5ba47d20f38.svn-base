...
function salvar(acao) {
  if (document.all.idLoja.value == '0') {
    ...
    document.forms[0].action='formCadastroItensPedidoSaida.jsp';
  	document.forms[0].submit();
  }
}
function cancelar() {
  document.forms[0].action="formPedidoSaida.jsp";
  document.forms[0].submit();
}
...
<h1 class="cabecalho_pagina">Movimentação de Saída</h1>
<form method="post" action="formPedidoSaida.jsp">
...
<table border="0" width="100%">
  <tr>
    ...
    <tr>
      <th class="label" style="height: 22px">Destinatário*</th>
          <td style="height: 15px"><input disabled type="text" name="dsEntidade" <%if (dsEntidade != null) { %>value="<%=dsEntidade%>"<% }%>  size="60" maxlength="60"> <a href="consultarClientes.jsp?acao=<%=acao%>&idPedidoSaida=<%=idPedidoSaida%>&idLoja=<%=idLoja%>&idEntrega=<%=idEntrega%>&idColaborador=<%=idColaborador%>"><img border = "0" src="../images/pesquisa.gif"></a></td>
  </tr>
    <tr>
  ...
  <tr>
    ...
    <td class="label_radio">
      ...
      <input disabled type="radio" class="radio" name="status" value="F" <%= (status.equals("F")? "checked": "") %>>Faturado
  ...
  <tr>
    ...
    <td class="label_radio">
      <input disabled type="radio" class="radio" name="operacao" value="S" <%= (operacao.equals("S")? "checked": "") %>>Saída
