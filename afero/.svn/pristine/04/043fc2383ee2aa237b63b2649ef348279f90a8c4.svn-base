...
function confirmaExcluir(cont, id) {  
  if (confirm("Deseja mesmo apagar o registro [" + cont + "]?")) {  
       location.href = 'listarEntidadeJuridica.jsp?acao=exc&cdEntidade=' + id;   
  }  
}
...
if (acao.equalsIgnoreCase("inc") || acao.equalsIgnoreCase("atu") || acao.equalsIgnoreCase("exc")) {
  ...
  if (acao.equalsIgnoreCase("inc")) {
    EntidadeJuridicaDAO daoPesquisaEntidade = new EntidadeJuridicaDAO(conn);
      ...
      response.sendRedirect("formEntidadeJuridica.jsp?acao=inc&mensagem="+mensagem);
    } else {
    ...
    EntidadeJuridicaDAO daoPesquisaEntidade = new EntidadeJuridicaDAO(conn);
      ...
      mensagem = "CNPJ já cadastrado";
      response.sendRedirect("formEntidadeJuridica.jsp?acao=atu&cdEntidade="+entidade.getCdEntidade()+"&mensagem="+mensagem);
    } else {
...
if (acao.equalsIgnoreCase("inc") && mensagem.isEmpty()) {
	response.sendRedirect("formEntidadeJuridica.jsp?acao=atu&cdEntidade=" + entidade.getCdEntidade());
}
...
|<a class="button" href="formEntidadeJuridica.jsp?acao=inc">Novo Cadastro</a>|<br>
<hr>
<body onload="document.forms[0].elements[0].focus();" >
<form action="listarEntidadeJuridica.jsp">
<table colspan="2">
  <tr>
...
%>
    <tr>
      ...
      <td class = "grid" width="30%"><center><a href="formEntidadeJuridica.jsp?acao=atu&cdEntidade=<%=entidadeJuridica.getCdEntidade()%>"><%=entidadeJuridica.getNome()%></a></center></td>
...
<p align="right">|<a class="button" href="formEntidadeJuridica.jsp?acao=inc">Novo Cadastro</a>|
<%  if (rowNum != 0 || hasNext) { %>
        <center>
<%      if (rowNum != 0) { %>
<a class="link" href="listarEntidadeJuridica.jsp?rowNum=<%=0%>&nomeLocalizar=<%=nomeLocalizar%>&cnpjLocalizar=<%=cnpjLocalizar%>&telefoneLocalizar=<%=telefoneLocalizar%>&statusLocalizar=<%=statusLocalizar%>">Primeira</a>&nbsp&nbsp
|&nbsp&nbsp<a class="link" href="listarEntidadeJuridica.jsp?rowNum=<%=rowNum-gridSize%>&nomeLocalizar=<%=nomeLocalizar%>&cnpjLocalizar=<%=cnpjLocalizar%>&telefoneLocalizar=<%=telefoneLocalizar%>&statusLocalizar=<%=statusLocalizar%>">< Anterior</a>&nbsp&nbsp
<%      } else { %>
Primeira&nbsp&nbsp|&nbsp&nbsp< Anterior&nbsp
<%      } %>
<%      if (hasNext) { %>
|&nbsp&nbsp<a class="link" href="listarEntidadeJuridica.jsp?rowNum=<%=rowNum+gridSize%>&nomeLocalizar=<%=nomeLocalizar%>&cnpjLocalizar=<%=cnpjLocalizar%>&telefoneLocalizar=<%=telefoneLocalizar%>&statusLocalizar=<%=statusLocalizar%>">Próxima ></a>&nbsp&nbsp
|&nbsp&nbsp<a class="link" href="listarEntidadeJuridica.jsp?rowNum=<%=rowCount-gridSize%>&nomeLocalizar=<%=nomeLocalizar%>&cnpjLocalizar=<%=cnpjLocalizar%>&telefoneLocalizar=<%=telefoneLocalizar%>&statusLocalizar=<%=statusLocalizar%>">Última</a>
<%      } else { %>
