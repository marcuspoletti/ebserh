...
%>
    <tr>
      ...
      <td class = "grid"><center><a class="link" target="principal" href="formPedido.jsp?cdEntidade=<%=cdEntidade%>&acao=<%=acao%>&dsEntidade=<%=nomeEntidade%>&idLoja=<%=idLoja%>&idEntrega=<%=idEntrega%>&idColaborador=<%=idColaborador%>&cdCondPagto=<%=cdCondPagto%>"onclick="javascript:window.close()"><%=nomeEntidade%></a></center></td>
