...
function localizar() {
  document.forms[0].action="maisVendidos.jsp";
...
<%
MaisVendidos maisVendidos = null;
...
String clausula = " where pss.status='A' and pss.operacao='S' and pss.idLoja = " + idLoja;
...
list = dao.maisVendidos(clausula);

%>

<h1 class="cabecalho_pagina">Produtos Mais Vendidos</h1>

<hr>

<body onload="document.forms[0].elements[0].focus();" >
<form action="maisVendidos.jsp">
...
<table border="0" width="100%">
    <tr">
      ...
      <th class="grid"><center>Total Vendidos</center></th>
...
for ( Iterator it = list.iterator(); hasNext && cont < gridSize; hasNext = it.hasNext()) {
	maisVendidos = (MaisVendidos) it.next();
	cont++;
%>
    <tr>
      <td class = "grid" width="5%"><center><%=rowNum+cont%></center></td>
      <td class = "grid" width="30%"><center><%=maisVendidos.getNmProduto()%></center></td>
      <td class = "grid" width="20%"><center><%=Utilitaria.formatarNumero(maisVendidos.getQuant(),3) %></center></td>
  </tr>    
<%
}
...
<%  if (rowNum != 0 || hasNext) { %>
        <center>
<%      if (rowNum != 0) { %>
<a class="link" href="maisVendidos.jsp?rowNum=<%=0%>&idLoja=<%=idLoja%>&dataIni=<%=dataIni%>&dataFim=<%=dataFim%>">Primeira</a>&nbsp&nbsp
|&nbsp&nbsp<a class="link" href="maisVendidos.jsp?rowNum=<%=rowNum-gridSize%>&idLoja=<%=idLoja%>&dataIni=<%=dataIni%>&dataFim=<%=dataFim%>">< Anterior</a>&nbsp&nbsp
<%      } else { %>
Primeira&nbsp&nbsp|&nbsp&nbsp< Anterior&nbsp
<%      } %>
<%      if (hasNext) { %>
|&nbsp&nbsp<a class="link" href="maisVendidos.jsp?rowNum=<%=rowNum+gridSize%>&idLoja=<%=idLoja%>&dataIni=<%=dataIni%>&dataFim=<%=dataFim%>">Próxima ></a>&nbsp&nbsp
|&nbsp&nbsp<a class="link" href="maisVendidos.jsp?rowNum=<%=rowCount-gridSize%>&idLoja=<%=idLoja%>&dataIni=<%=dataIni%>&dataFim=<%=dataFim%>">Última</a>
<%      } else { %>
