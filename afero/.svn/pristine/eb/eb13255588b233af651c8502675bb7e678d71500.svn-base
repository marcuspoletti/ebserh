...
if(request.getParameter("idLoja")!= null){
	cdLoja = request.getParameter("idLoja");
}
int idLoja = Integer.parseInt(cdLoja);
if (nomeProduto!= null) {
...
if (!status.isEmpty()) {
  if (clausula.isEmpty()) {
	  if(status.equals("T")){
		  clausula = " WHERE status is not null";	  
	  }else{
...
<h1 class="cabecalho_pagina">Consulta de Movimentação de Estoque</h1>

<hr>

<body onload="document.forms[0].elements[0].focus();" >
<form action="consultarProduto.jsp?acao=atu">
...
<table colspan="2">
  ...
  <tr>
    ...
    <td class="label_radio">
      <input type="radio" class="radio" name="status" value="T" <%= (status.equals("T")? "checked": "") %>>Todos
...
<table border="0" width="100%">
    <tr>
      ...
      <th class="grid"><center>Ent.</center></th>
      <th class="grid"><center>Saída.</center></th>
...
%>
    <tr>
     ...
     <td class = "grid" width="2%"><a target="_blank" href="entradasProduto.jsp?idProduto=<%=prod.getIdProduto()%>"><img border="0"src="../images/edit.gif"></img></a></td>
     <td class = "grid" width="2%"><a target="_blank" href="saidaProduto.jsp?idProduto=<%=prod.getIdProduto()%>"><img border="0"src="../images/edit.gif"></img></a></td>
...
<%  if (rowNum != 0 || hasNext) { %>
        <center>
<%      if (rowNum != 0) { %>
<a class="link" href="consultarProduto.jsp?rowNum=<%=0%>&nomeProduto=<%=nomeProduto%>&status=<%=status%>&idLoja=<%=idLoja%>">Primeira</a>&nbsp&nbsp
|&nbsp&nbsp<a class="link" href="consultarProduto.jsp?rowNum=<%=rowNum-gridSize%>&nomeProduto=<%=nomeProduto%>&status=<%=status%>&idLoja=<%=idLoja%>">< Anterior</a>&nbsp&nbsp
<%      } else { %>
Primeira&nbsp&nbsp|&nbsp&nbsp< Anterior&nbsp
<%      } %>
<%      if (hasNext) { %>
|&nbsp&nbsp<a class="link" href="consultarProduto.jsp?rowNum=<%=rowNum+gridSize%>&nomeProduto=<%=nomeProduto%>&status=<%=status%>&idLoja=<%=idLoja%>">Próxima ></a>&nbsp&nbsp
|&nbsp&nbsp<a class="link" href="consultarProduto.jsp?rowNum=<%=rowCount-gridSize%>&nomeProduto=<%=nomeProduto%>&status=<%=status%>&idLoja=<%=idLoja%>">Última</a>
<%      } else { %>
