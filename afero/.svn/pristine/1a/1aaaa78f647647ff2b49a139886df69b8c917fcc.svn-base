...
ConfigGeralDAO daoConfigGeral = new ConfigGeralDAO(conn);
String integracao= daoConfigGeral.procurarIntegracaoPedSaidaFinan(idLoja);
if (acao.equalsIgnoreCase("gravar")){
	...
	if (!pedidoSaidaUpdate.getStatus().equals("A")) {
			...
			if(status.equalsIgnoreCase("A")){
				...
				if(integracao.equalsIgnoreCase("S")){
					daoPedidoSaidaUpdate.atualizarContasReceber(pedidoSaidaUpdate);
				}
			}
	} else {
