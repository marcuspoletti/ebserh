...
function confirmaExcluir(cont, id) {  
  if (confirm("Deseja mesmo apagar o registro [" + cont + "]?")) {  
       location.href = 'listarEntidadeFisica.jsp?acao=exc&cdEntidade=' + id;   
  }  
}
...
if (acao.equalsIgnoreCase("inc") || acao.equalsIgnoreCase("atu") || acao.equalsIgnoreCase("exc")) {
  ...
  if (acao.equalsIgnoreCase("inc")) {
	...
	if (!cpfCnpj.equals("000.000.000-00") && daoPesquisaEntidade.procurarCpf(cpfCnpj) != 0 ) { 
      ...
      response.sendRedirect("formEntidadeFisica.jsp?acao=inc&mensagem="+mensagem);
    }else {
    ...
    EntidadeFisicaDAO daoPesquisaEntidade = new EntidadeFisicaDAO(conn);
      ...
      mensagem = "CPF já cadastrado";
      response.sendRedirect("formEntidadeFisica.jsp?acao=atu&cdEntidade="+entidade.getCdEntidade()+"&mensagem="+mensagem);
    } else {
...
if (acao.equalsIgnoreCase("inc") && mensagem.isEmpty()) {
	response.sendRedirect("formEntidadeFisica.jsp?acao=atu&cdEntidade=" + entidade.getCdEntidade());
}
...
|<a class="button" href="formEntidadeFisica.jsp?acao=inc">Novo Cadastro</a>|<br>
<hr>
<body onload="document.forms[0].elements[0].focus();" >
<form action="listarEntidadeFisica.jsp">
<table colspan="2">
  <tr>
...
%>
    <tr>
      ...
      <td class = "grid"><center><a href="formEntidadeFisica.jsp?acao=atu&cdEntidade=<%=entidadeFisica.getCdEntidade()%>"><%=entidadeFisica.getNome()%></a></center></td>
...
|<a class="button" href="../exportacao/formGerarArquivoCliente.jsp?acao=exp">Exportar</a>|  </div>
  <div style="width:50%; text-align: right; float:left;">
|<a class="button" href="formEntidadeFisica.jsp?acao=inc">Novo Cadastro</a>|
  </div>
</div>
<%  if (rowNum != 0 || hasNext) { %>
        <center>
<%      if (rowNum != 0) { %>
<a class="link" href="listarEntidadeFisica.jsp?rowNum=<%=0%>&nomeLocalizar=<%=nomeLocalizar%>&cpfLocalizar=<%=cpfLocalizar%>&telefoneLocalizar=<%=telefoneLocalizar%>&statusLocalizar=<%=statusLocalizar%>">Primeira</a>&nbsp&nbsp
|&nbsp&nbsp<a class="link" href="listarEntidadeFisica.jsp?rowNum=<%=rowNum-gridSize%>&nomeLocalizar=<%=nomeLocalizar%>&cpfLocalizar=<%=cpfLocalizar%>&telefoneLocalizar=<%=telefoneLocalizar%>&statusLocalizar=<%=statusLocalizar%>">< Anterior</a>&nbsp&nbsp
<%      } else { %>
Primeira&nbsp&nbsp|&nbsp&nbsp< Anterior&nbsp
<%      } %>
<%      if (hasNext) { %>
|&nbsp&nbsp<a class="link" href="listarEntidadeFisica.jsp?rowNum=<%=rowNum+gridSize%>&nomeLocalizar=<%=nomeLocalizar%>&cpfLocalizar=<%=cpfLocalizar%>&telefoneLocalizar=<%=telefoneLocalizar%>&statusLocalizar=<%=statusLocalizar%>">Próxima ></a>&nbsp&nbsp
|&nbsp&nbsp<a class="link" href="listarEntidadeFisica.jsp?rowNum=<%=rowCount-gridSize%>&nomeLocalizar=<%=nomeLocalizar%>&cpfLocalizar=<%=cpfLocalizar%>&telefoneLocalizar=<%=telefoneLocalizar%>&statusLocalizar=<%=statusLocalizar%>">Última</a>
<%      } else { %>
