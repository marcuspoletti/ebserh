...
if (acao.equalsIgnoreCase("gravar")){
	...
	//if (!daoPedidoSaidaUpdate.confirmarPedido(Integer.parseInt(idPedidoSaida))) {
		...
		response.sendRedirect("exibirPedidoSaida.jsp?mensagem=Já foi aprovado este Pedido&idPedidoSaida=" + idPedidoSaida);
	}
	//} else {
	//	response.sendRedirect("exibirPedidoSaida.jsp?mensagem=Não pode aprovar o pedido porque a Ordem de Serviço esta aberta&idPedidoSaida=" + idPedidoSaida);
	//}
	acao = "listar";
}
%>
...
<form method="post" action="exibirPedidoSaida.jsp?acao=<%="gravar"%>">
...
%>
    <tr>
      ...
      <td class="grid" width="3%" name="quantidade" size="10" maxlength="10"><%=Utilitaria.formatarNumero(pedidoSaidaItem.getQuant(),3)%></td>
...
%> 
  ...
  <%
    valorTotal = total+valorEntrega;
  %>
  <tr>
      <th class="grid" width="5%"><center>VALOR TOTAL:</center></th>
      <td><input type="text" id="valorTotal" value="<%=Utilitaria.formatarNumero((valorTotal),2)%>" onblur="soma()"></td>
  </tr>
  <br>
</table>
<br>
