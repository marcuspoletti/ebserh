...
<%
	...
	if(entidade.getTpInsc().equalsIgnoreCase("F")){
		...
		EntidadeJuridicaDAO daoJuridica = new  EntidadeJuridicaDAO(conn);
		EntidadeJuridica juridica = daoJuridica.procurarEntidadeJuridica(cdEntidade);
	}
	...
	textoBobina.append("========================================\n");
	textoBobina.append("==============Presencial================\n");
	textoBobina.append("Pes. Resp:.: ").append((String) pedidoSaida.getPessoaResponsavel()).append("\n");
	textoBobina.append("Operador...: ").append((String) pedidoSaida.getUsuario()).append("\n");
	textoBobina.append("Dt Pedido..: ").append(dtPed).append("\n");
	textoBobina.append("Dt Entrega.: ").append(dtEntrega).append("\n");
	textoBobina.append("Hora Ent...: ").append(hrEntrega).append("\n");
	...
	while(ok){
		if(referencia.length() > I){
			textoReferencia = referencia.substring(inicio, I);
			textoBobina.append(textoReferencia).append("\n");
			textoReferencia = "";
			inicio = I;
			I = (I*2);
		}else{
			textoReferencia = referencia.substring(inicio, referencia.length());
			textoBobina.append(textoReferencia).append("\n");
			textoReferencia = "";
			ok = false;
		}
	...
	textoBobina.append("========================================\n");
	textoBobina.append("OBSERVACAO------------------------------\n");
	textoBobina.append(observacao).append("\n");
	textoBobina.append("========================================\n");
	textoBobina.append("========================================\n");
				...
				for (Iterator itListarSubItens = listarEstoqueSubItens.iterator(); itListarSubItens.hasNext();) {
					...
					if (precoListarSubList != null) {
						...
						if (pedidoSubItens.getDsCompProduto() == null) {
							descricao = Utilitaria.abreviarStr(prodSubItens.getDsProduto(),	40).trim();
						} else {
							descricao = Utilitaria.abreviarStr(prodSubItens.getDsProduto(),	40).trim();
							complemento = Utilitaria.abreviarStr(pedidoSubItens.getDsCompProduto(),	40);
						}
						if (descricao.trim().length()>40) {
							textoBobina.append(cb(descricao.substring(0,39), 40)
									+ cb(Utilitaria.formatarNumero(pedidoSubItens.getQuant(),	2), 6)
									+ "\n");
							textoBobina.append(cb(descricao.substring(40), 40)
									+ "\n");
						} else {
							textoBobina.append(cb(descricao, 40)
									+ cb(Utilitaria.formatarNumero(pedidoSubItens.getQuant(),	2), 6)
									+ "\n");
						}
						subItemOk = true;
	...
	if (!subItemOk) {
		...
		if (pedidoSaidaItem.getDsCompProduto() == null) {
			...
			descricao = Utilitaria.abreviarStr(prodSubItens.getDsProduto(),	40).trim();
			complemento = Utilitaria.abreviarStr(pedidoSaidaItem.getDsCompProduto(),	40);
		...
		if (descricao.trim().length()>40) {
			...
			if(complemento.length() > 2){
				textoBobina.append(cb(complemento.substring(0,39), 40)+ "\n");
			}
			
			textoBobina.append("                    "
				+ cb(Utilitaria.formatarNumero(pedidoSaidaItem.getQuant(),	2), 6)
				+ cb(Utilitaria.formatarNumero(pedidoSaidaItem.getVlUni(),	2), 6)
				+ cb(Utilitaria.formatarNumero(pedidoSaidaItem.getQuant()*pedidoSaidaItem.getVlUni(),	2), 9)
				+ "\n");
			textoBobina.append(cb(descricao.substring(40), 40)
				+ "\n");
		} else {
			textoBobina.append(cb(descricao, 40)+ "\n");
			if(complemento.length() > 2){
				textoBobina.append(cb(complemento, 40)+ "\n");
			}
			textoBobina.append("                    "
				+ cb(Utilitaria.formatarNumero(pedidoSaidaItem.getQuant(),	2), 6)
				+ cb(Utilitaria.formatarNumero(pedidoSaidaItem.getVlUni(),	2), 6)
...
//	textoBobina.append("Cond.Pagto.:" + condPagto.getDsCondPagto() + "\n");
	textoBobina.append("OBS / TROCO:").append("\n");
	String textoObrservacao = "";
	int J=40;
	int ini = 0;
	boolean okObs = true;
	while(okObs){
		if(observacao.length() > J){
			textoObrservacao = observacao.substring(ini, J);
			textoBobina.append(textoObrservacao).append("\n");
			textoObrservacao = "";
			ini = J;
			J = (J*2);
		}else{
			textoObrservacao = observacao.substring(ini, observacao.length());
			textoBobina.append(textoObrservacao).append("\n");
			textoObrservacao = "";
			okObs = false;
		}
...
%>
 ...
 <table>
   <tr>
      <td align="center"><a href="impressaoBobina.jsp?idPedidoSaida=<%=idPedidoSaida%>">Imprimir</a></td>
   </tr>
 </table>
<pre style="font-size: 12pt; font-weight: bold;" id="texto_bobina">
<%=textoBobina.toString()%>
<%bobina.imprime(textoBobina.toString());%>
<%bobina.acionarGuilhotina();%>
</pre>
</body></html>
