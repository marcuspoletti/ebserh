...
|<a class="button" href="cadastros/formEntidadeFisicaSimplificado.jsp?acao=inc">Novo Cadastro</a>|<br>
<hr>
...
for ( Iterator it = list.iterator(); it.hasNext(); ) {
	if(tpInsc.equalsIgnoreCase("F")){
		...
		listarTelefone = daoEntidadeFone.listarTelefone(cdEntidade);
		EntidadeEmailDAO emailDAO = new EntidadeEmailDAO(conn);
		email = emailDAO.procurarEntidadeEmailPadrao(cdEntidade);
		int contador = 0;
		for ( Iterator itTelefone = listarTelefone.iterator(); itTelefone.hasNext(); ) {
			...
			if(entidadeFone.getNroTelefone() != null){
				//contador++;
				//if(contador <= 2){
					tel = telefone + " " +entidadeFone.getNroTelefone();
				//}
			}
		}
		...
		idEntrega = "".valueOf(entidadeEndereco.getIdEntrega());
		cidadeUf = "";
		if(cep == null)cep="";
		...
		if(contatoDAO.pesquisarEntidadeContato(cdEntidade)){
		 EntidadeContato entidadeContato = contatoDAO.procurarEntidadeContato(cdEntidade);
...
%>
    <tr>
      ...
      <td class = "grid" width="30%"><center><a class="link" target="principal" href="formPedido.jsp?cdCondPagto=<%=cdCondPagto%>&cdEntidade=<%=cdEntidade%>&acao=<%=acao%>&dsEntidade=<%=nomeEntidade%>&idLoja=<%=idLoja%>&idEntrega=<%=idEntrega%>&idColaborador=<%=idColaborador%>&referencia=<%=referencia%>"onclick="javascript:window.close()"><%=nomeEntidade%></a></center></td>
      ...
      <td class = "grid" width="15%"><center><a href="cadastros/formEntidadeFisicaSimplificado.jsp?acao=atu&cdEntidade=<%=cdEntidade%>"><%=tel%></a></center></td>
      <td class = "grid" width="15%"><center><%=contato%></center></td>
...
<p align="right">|<a class="button" href="cadastros/formEntidadeFisicaSimplificado.jsp?acao=inc">Novo Cadastro</a>|
<%  if (rowNum != 0 || hasNext) { %>
        <center>
<%      if (rowNum != 0) { %>
<a class="link" href="consultarClientes.jsp?rowNum=<%=0%>&cdCondPagto=<%=cdCondPagto %>&idColaborador=<%=idColaborador %>&idEntrega=<%=idEntrega %>&idLoja=<%=idLoja %>&idPedidoSaida=<%=idPedidoSaida %>&nomeLocalizar=<%=nomeLocalizar%>&statusLocalizar=<%=statusLocalizar%>&tpInsc=<%=tpInsc%>&cpfLocalizar=<%=cpfLocalizar%>&telefone=<%=telefone%>&dsEndereco=<%=dsEndereco%>&cdTipoLogradouro=<%=cdTipoLogradouro%>">Primeira</a>&nbsp&nbsp
|&nbsp&nbsp<a class="link" href="consultarClientes.jsp?rowNum=<%=rowNum-gridSize%>&cdCondPagto=<%=cdCondPagto %>&idColaborador=<%=idColaborador %>&idEntrega=<%=idEntrega %>&idLoja=<%=idLoja %>&idPedidoSaida=<%=idPedidoSaida %>&nomeLocalizar=<%=nomeLocalizar%>&statusLocalizar=<%=statusLocalizar%>&tpInsc=<%=tpInsc%>&cpfLocalizar=<%=cpfLocalizar%>&telefone=<%=telefone%>&dsEndereco=<%=dsEndereco%>&cdTipoLogradouro=<%=cdTipoLogradouro%>">< Anterior</a>&nbsp&nbsp
<%      } else { %>
Primeira&nbsp&nbsp|&nbsp&nbsp< Anterior&nbsp
<%      } %>
<%      if (hasNext) { %>
|&nbsp&nbsp<a class="link" href="consultarClientes.jsp?rowNum=<%=rowNum+gridSize%>&cdCondPagto=<%=cdCondPagto %>&idColaborador=<%=idColaborador %>&idEntrega=<%=idEntrega %>&idLoja=<%=idLoja %>&idPedidoSaida=<%=idPedidoSaida %>&nomeLocalizar=<%=nomeLocalizar%>&statusLocalizar=<%=statusLocalizar%>&tpInsc=<%=tpInsc%>&cpfLocalizar=<%=cpfLocalizar%>&telefone=<%=telefone%>&dsEndereco=<%=dsEndereco%>&cdTipoLogradouro=<%=cdTipoLogradouro%>">Próxima ></a>&nbsp&nbsp
|&nbsp&nbsp<a class="link" href="consultarClientes.jsp?rowNum=<%=rowCount-gridSize%>&cdCondPagto=<%=cdCondPagto %>&idColaborador=<%=idColaborador %>&idEntrega=<%=idEntrega %>&idLoja=<%=idLoja %>&idPedidoSaida=<%=idPedidoSaida %>&nomeLocalizar=<%=nomeLocalizar%>&statusLocalizar=<%=statusLocalizar%>&tpInsc=<%=tpInsc%>&cpfLocalizar=<%=cpfLocalizar%>&telefone=<%=telefone%>&dsEndereco=<%=dsEndereco%>&cdTipoLogradouro=<%=cdTipoLogradouro%>">Última</a>
<%      } else { %>
