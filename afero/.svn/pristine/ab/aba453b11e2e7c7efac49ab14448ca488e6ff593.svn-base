...
<%
	...
	boolean ok = true;
	while(ok){
		if(anotacao.length() > I){
			textoAnotacao = anotacao.substring(inicio, I);
			textoBobina.append(textoAnotacao).append("\n");
			textoAnotacao = "";
			inicio = I;
			I = (I*2);
		}else{
			textoAnotacao = anotacao.substring(inicio, anotacao.length());
			textoBobina.append(textoAnotacao).append("\n");
			textoAnotacao = "";
			ok = false;
		}
				...
				for (Iterator itListarSubItens = listarEstoqueSubItens.iterator(); itListarSubItens.hasNext();) {
					...
					if (precoListarSubList != null) {
						...
						if (pedidoSubItens.getDsCompProduto() == null) {
							...
							descricao   = Utilitaria.abreviarStr(prodSubItens.getDsProduto(),	40).trim();
							if (pedidoSaidaItem.getDsCompProduto().length() > 2){
								complemento = Utilitaria.abreviarStr(pedidoSubItens.getDsCompProduto(),	40).trim();
							}
						}
						if (descricao.trim().length()>40) {
							textoBobina.append(cb(descricao.substring(0,39), 40)+ "\n");
							textoBobina.append(cb(Utilitaria.formatarNumero(pedidoSubItens.getQuant(),	2), 6)
									+ "\n");
	...
	if (!subItemOk) {
		...
		if (pedidoSaidaItem.getDsCompProduto() == null) {
			...
			descricao = Utilitaria.abreviarStr(prodSubItens.getDsProduto(),	40).trim();
                           complemento = Utilitaria.abreviarStr(pedidoSaidaItem.getDsCompProduto(),	40).trim();
		}
		float precoCalc = pedidoSaidaItem.getQuant()*pedidoSaidaItem.getVlUni();
		if (descricao.trim().length()>40) {
			textoBobina.append(cb(descricao.substring(0,39), 40)+ "\n");
			...
			textoBobina.append("                    "
				...
				+ cb(Utilitaria.formatarNumero(precoCalc,	2), 9)
				+ "\n");
			textoBobina.append(cb(descricao.substring(40), 40)
				+ "\n");
		} else {
			...
			precoCalc = pedidoSaidaItem.getQuant()*pedidoSaidaItem.getVlUni();
			textoBobina.append("                    "
				+ cb(Utilitaria.formatarNumero(pedidoSaidaItem.getQuant(),	2), 6)
				+ cb(Utilitaria.formatarNumero(pedidoSaidaItem.getVlUni(),	2), 6)
...
%> 
   <tr>
      <td align="center"><a href="impressaoBobina.jsp?idPedidoSaida=<%=idPedidoSaida%>"><b>Imprimir</b></a></td>
   </tr>
<% 
  }
%>
