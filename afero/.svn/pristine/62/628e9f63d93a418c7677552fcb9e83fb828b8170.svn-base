...
%>
    <tr>
      ...
      <td class = "grid"><center><a class="link" target="principal" href="formOrcamento.jsp?cdEntidade=<%=cdEntidade%>&acao=<%=acao%>&dsEntidade=<%=nomeEntidade%>&idLoja=<%=idLoja%>&idEntrega=<%=idEntrega%>&idColaborador=<%=idColaborador%>"onclick="javascript:window.close()"><%=nomeEntidade%></a></center></td>
