...
%>
    <tr>
      ...
      <td class = "grid"><center><a class="link" target="principal" href="formCompras.jsp?cdEntidade=<%=cdEntidade%>&acao=<%=acao%>&dsEntidade=<%=nomeEntidade%>&idLoja=<%=idLoja%>&idColaborador=<%=idColaborador%>&nrDoc=<%=nrDoc%>"onclick="javascript:window.close()"><%=nomeEntidade%></a></center></td>
