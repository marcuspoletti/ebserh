...
String cdEntidade = sdEntidade;
if(sdEntidade == "")sdEntidade="0";
int cdEntidadePedido = Integer.parseInt(sdEntidade);
...
if(cdTipoLogradouro == null)cdTipoLogradouro="1";
...
if (acao.equalsIgnoreCase("inc") || acao.equalsIgnoreCase("atu") || acao.equalsIgnoreCase("exc")) {
  ...
  if (idTipoEntidade == null) idTipoEntidade = "1";
  if (nome == null) nome = "";
  ...
  if (dtMod == null) dtMod = "";
  if (dtCad == null) dtCad = "";
  if (cpfCnpj == null) cpfCnpj = "000.000.000-00";
  ...
  cdTipoLogradouro = request.getParameter("cdTipoLogradouro");
  String idCidade = request.getParameter("idCidade");
  dsEndereco = request.getParameter("dsEndereco");
  ...
  EntidadeEmail entidadeEmail = new EntidadeEmail();
    ...
    entidadeFisica.setPai(pai);
	  ...
	  //Atualização 16/04/2012 Pizzaria
	  entidadeEndereco.setIdEntrega(Integer.parseInt(idEntrega));
  ...
  if (acao.equalsIgnoreCase("inc")) {
    EntidadeFisicaDAO daoPesquisaEntidade = new EntidadeFisicaDAO(conn);
      ...
      response.sendRedirect("formEntidadeFisicaSimplificado.jsp?acao=inc&mensagem="+mensagem);
    	...
    	dao2.incluirPizzaria(entidadeEndereco);
    ...
    EntidadeFisicaDAO daoPesquisaEntidade = new EntidadeFisicaDAO(conn);
      ...
      response.sendRedirect("formEntidadeFisicaSimplificado.jsp?acao=atu&cdEntidade="+entidade.getCdEntidade()+"&mensagem="+mensagem);
	  	...
	  	dao2.atualizarPizzaria(entidadeEndereco);
...
if (nomeLocalizar!= null) {
  ...
  if (!statusLocalizar.equals("T")) {
	...
	if (!cdTipoLogradouro.equalsIgnoreCase("1")) {
		  if (clausula.isEmpty()) {
		    clausula += " WHERE end.cdTipoLogradouro= "+Integer.parseInt(cdTipoLogradouro);
		  } else {
		    clausula = clausula+" AND end.cdTipoLogradouro= "+Integer.parseInt(cdTipoLogradouro);
		  }
	}
	if (dsEndereco != null) {
		  if (clausula.isEmpty()) {
		    clausula += " WHERE end.dsEndereco like'"+dsEndereco+"%'";
		  } else {
		    clausula = clausula+" AND end.dsEndereco like'"+dsEndereco+"%'";
		  }
	}
}
...
%>
<h1 class="cabecalho_pagina">Cadastro de Entidade Física (Comandas)</h1>
...
<table border="0" width="100%">
    <tr">
      ...
      <th class="grid"><center>Ref.</center></th>
      <th class="grid"><center>Ped. Ant.</center></th> 
...
String cidadeUf = "";
String tel = "";
EntidadeEmail email = null;
List listarTelefone;
//Utiliza o ResultSet para trazer os registros do banco de dados
for ( Iterator it = list.iterator(); hasNext && cont < gridSize; hasNext = it.hasNext()) {
	
	if(tpInsc.equalsIgnoreCase("F")){
		EntidadeFisica entidadeF = (EntidadeFisica) it.next();
		cpfCnpj = entidadeF.getCpf();
		cdEntidade = "".valueOf(entidadeF.getCdEntidade());
		nomeEntidade = entidadeF.getNome();
		statusEntidade = entidadeF.getStatus();
		...
		entidadeEndereco = daoEntidadeEndereco.procurarEntidadeEnderecoPadraoPizzaria(Integer.parseInt(cdEntidade));
		EntidadeTelefoneDAO daoEntidadeFone = new EntidadeTelefoneDAO(conn);
		EntidadeTelefone entidadeFone = null;
		listarTelefone = daoEntidadeFone.listarTelefone(Integer.parseInt(cdEntidade));
		EntidadeEmailDAO emailDAO = new EntidadeEmailDAO(conn);
		email = emailDAO.procurarEntidadeEmailPadrao(Integer.parseInt(cdEntidade));
		int contador = 0;
...
%>
    <tr>
      ...
      <td class = "grid"><center><a class="link" target="principal" href="../formPedido.jsp?referencia=<%=entidadeEndereco.getRefEndereco()%>&cdCondPagto=<%=cdCondPagto%>&cdEntidade=<%=entidadeF.getCdEntidade()%>&acao=<%=acao%>&dsEntidade=<%=entidadeF.getNome()%>&idLoja=<%=idLoja%>&idEntrega=<%=entidadeEndereco.getIdEntrega()%>&idColaborador=<%=idColaborador%>"onclick="javascript:window.close()"><%=entidadeF.getNome()%></a></center></td>
      <%-- <td class = "grid" width="10%"><center><%=entidadeF.getCpf()%></center></td> --%>
      <td class = "grid" width="30%"><center><a class="link" target="_blank" href="../../cadastros/formEntidadeFisica.jsp?acao=atu&cdEntidade=<%=entidadeF.getCdEntidade()%>"><%=endereco +" Bairro: "+bairro + " Cep:"+cep%></a></center></td>
      <%-- <td class = "grid" width="15%"><center><%=tel%></center></td> --%>
      <td class = "grid" width="15%"><center><a href="formEntidadeFisicaSimplificado.jsp?acao=atu&cdEntidade=<%=entidadeF.getCdEntidade()%>"><%=tel%></a></center></td>
      <td class = "grid" width="15%"><center><%=entidadeEndereco.getRefEndereco()%></center></td>
      <td class = "grid" width="5%"><center><a target="_blank" href="../listarPedidosVendidos.jsp?cdEntidade=<%=entidadeF.getCdEntidade()%>"><img border = "0" src="../../images/ficha.gif"></a></center></td>
      <%--<td class = "grid" width="15"><center><%=email.getDsEmail()%></center></td> --%>
      <td class = "grid" width="5%"><center><%=entidadeF.getStatus()%></center></td>
     <!-- <td class = "grid" width="3%"><center><a href="javascript: confirmaExcluir(<%=rowNum+cont%>,<%=entidadeF.getCdEntidade()%>);"><img border = "0" src="../images/delete.gif"></a></center></td> -->
...
<%  if (rowNum != 0 || hasNext) { %>
        <center>
<%      if (rowNum != 0) { %>
<a class="link" href="listarEntidadeFisicaSimplificado.jsp?rowNum=<%=0%>&cdCondPagto=<%=cdCondPagto %>&idColaborador=<%=idColaborador %>&idEntrega=<%=idEntrega %>&idLoja=<%=idLoja %>&idPedidoSaida=<%=idPedidoSaida %>&nomeLocalizar=<%=nomeLocalizar%>&cpfLocalizar=<%=cpfLocalizar%>&telefoneLocalizar=<%=telefoneLocalizar%>&statusLocalizar=<%=statusLocalizar%>">Primeira</a>&nbsp&nbsp
|&nbsp&nbsp<a class="link" href="listarEntidadeFisicaSimplificado.jsp?rowNum=<%=rowNum-gridSize%>&cdCondPagto=<%=cdCondPagto %>&idColaborador=<%=idColaborador %>&idEntrega=<%=idEntrega %>&idLoja=<%=idLoja %>&idPedidoSaida=<%=idPedidoSaida %>&nomeLocalizar=<%=nomeLocalizar%>&cpfLocalizar=<%=cpfLocalizar%>&telefoneLocalizar=<%=telefoneLocalizar%>&statusLocalizar=<%=statusLocalizar%>">< Anterior</a>&nbsp&nbsp
<%      } else { %>
Primeira&nbsp&nbsp|&nbsp&nbsp< Anterior&nbsp
<%      } %>
<%      if (hasNext) { %>
|&nbsp&nbsp<a class="link" href="listarEntidadeFisicaSimplificado.jsp?rowNum=<%=rowNum+gridSize%>&cdCondPagto=<%=cdCondPagto %>&idColaborador=<%=idColaborador %>&idEntrega=<%=idEntrega %>&idLoja=<%=idLoja %>&idPedidoSaida=<%=idPedidoSaida %>&nomeLocalizar=<%=nomeLocalizar%>&cpfLocalizar=<%=cpfLocalizar%>&telefoneLocalizar=<%=telefoneLocalizar%>&statusLocalizar=<%=statusLocalizar%>">Próxima ></a>&nbsp&nbsp
|&nbsp&nbsp<a class="link" href="listarEntidadeFisicaSimplificado.jsp?rowNum=<%=rowCount-gridSize%>&cdCondPagto=<%=cdCondPagto %>&idColaborador=<%=idColaborador %>&idEntrega=<%=idEntrega %>&idLoja=<%=idLoja %>&idPedidoSaida=<%=idPedidoSaida %>&nomeLocalizar=<%=nomeLocalizar%>&cpfLocalizar=<%=cpfLocalizar%>&telefoneLocalizar=<%=telefoneLocalizar%>&statusLocalizar=<%=statusLocalizar%>">Última</a>
<%      } else { %>
