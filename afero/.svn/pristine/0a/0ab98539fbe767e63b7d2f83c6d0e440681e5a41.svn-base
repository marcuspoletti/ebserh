...
int cont = 0;
%>
<h1 class="cabecalho_pagina">Consultar Clientes</h1>
<hr>
...
%>
    <tr>
      ...
      <td class = "grid"><center><a target="principal" href="editarFormPedido.jsp?idPedidoSaida=<%=idPedidoSaida%>&cdEntidade=<%=cdEntidade%>&acao=<%="atu"%>&dsEntidade=<%=nomeEntidade%>&idLoja=<%=idLoja%>&idEntrega=<%=idEntrega%>&idColaborador=<%=idColaborador%>"onclick="javascript:window.close()"><%=nomeEntidade%></a></center></td>
