...
String stCdCondPagto = "";
String clausula = "";

if(!cdCondPagto.equalsIgnoreCase("0")){
	clausula = " WHERE  ps.statusPag='"+statusPag+"' AND ps.tipoPedido ='"+tipoPedido+"' AND ps.status = '"+status+"' AND ps.dtEntrega BETWEEN '"+converte.DMYToYMDI(dataIni)+" 00:00:00' AND '"+converte.DMYToYMDI(dataFim)+" 23:59:59' ps.cdCondPagto = "+Integer.parseInt(cdCondPagto);
}else{
