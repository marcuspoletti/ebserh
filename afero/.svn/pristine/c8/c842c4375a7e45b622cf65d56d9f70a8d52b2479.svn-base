...
function localizar() {
  document.forms[0].action="produtosComMaiorSaidaPreco.jsp";
...
clausula += " group by idProduto order by preco desc ";
//seleciona todos os registros do banco de dados
List list = null;
List listarMaisVendido = null;
dao = new PedidoSaidaItemDAO(conn);
list = dao.comQuantidadeMaiorSaidaPreco(clausula);
...
<form action="produtosComMaiorSaidaPreco.jsp">
...
<table border="0" width="100%">
    <tr>
      ...
      <th class="grid"><center>Total (R$)</center></th>
...
%>
    <tr>
      ...
      <td class = "grid" width="30%"><center><%=maiorVenda.getQuant()%></center></td>
      <td class = "grid" width="20%"><center><%=Utilitaria.formatarNumero(maiorVenda.getPreco(),2) %></center></td>
  </tr>    
<%
}
...
<%  if (rowNum != 0 || hasNext) { %>
        <center>
<%      if (rowNum != 0) { %>
<a class="link" href="produtosComMaiorSaidaPreco.jsp?rowNum=<%=0%>&idLoja=<%=idLoja%>&dataIni=<%=dataIni%>&dataFim=<%=dataFim%>">Primeira</a>&nbsp&nbsp
|&nbsp&nbsp<a class="link" href="produtosComMaiorSaidaPreco.jsp?rowNum=<%=rowNum-gridSize%>&idLoja=<%=idLoja%>&dataIni=<%=dataIni%>&dataFim=<%=dataFim%>">< Anterior</a>&nbsp&nbsp
<%      } else { %>
Primeira&nbsp&nbsp|&nbsp&nbsp< Anterior&nbsp
<%      } %>
<%      if (hasNext) { %>
|&nbsp&nbsp<a class="link" href="produtosComMaiorSaidaPreco.jsp?rowNum=<%=rowNum+gridSize%>&idLoja=<%=idLoja%>&dataIni=<%=dataIni%>&dataFim=<%=dataFim%>">Próxima ></a>&nbsp&nbsp
|&nbsp&nbsp<a class="link" href="produtosComMaiorSaidaPreco.jsp?rowNum=<%=rowCount-gridSize%>&idLoja=<%=idLoja%>&dataIni=<%=dataIni%>&dataFim=<%=dataFim%>">Última</a>
<%      } else { %>
