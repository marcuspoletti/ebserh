...
function salvar(acao) {
   ...
   document.forms[0].action='formCadastroItensPedidoSubItem.jsp';
...
function trocar(acao) {
	   document.all.acao.value = 'transf';
	   document.forms[0].action='editarFormPedido.jsp';
	   document.forms[0].submit();
	}
</script>
</head>
...
String idMesaTroca = request.getParameter("idMesaTroca");
if(idMesa == null)idMesa="0";
if(idMesaTroca == null)idMesaTroca="0";
String quantPessoas = request.getParameter("quantPessoas");
if(quantPessoas == null)quantPessoas = "0";
...
String hrEntrega = "";
MesaDAO daoMesaTransf = new MesaDAO(conn);
if (acao.equalsIgnoreCase("transf")){
	// if(daoMesaTransf.verificarMesa(Integer.parseInt(idMesaTroca))){
		 idMesa = "".valueOf(daoMesaTransf.getIdMesa(Integer.parseInt(idPedidoSaida))); 
		 if(!idMesa.equalsIgnoreCase("0")){
			 daoMesaTransf.atualizarStatus(Integer.parseInt(idMesa), "L"); 
	          }
		daoMesaTransf.transfMesa(Integer.parseInt(idMesa), Integer.parseInt(idMesaTroca), Integer.parseInt(idPedidoSaida));
		daoMesaTransf.transfMesaItens(Integer.parseInt(idPedidoSaida), Integer.parseInt(idMesaTroca));
		daoMesaTransf.atualizarStatus(Integer.parseInt(idMesaTroca), "C");
	// }else{
	//	 mensagem = " Não é possível transferir a mesa !!! Porque a mesa selecionada esta com status de Consumindo ou Aberto .";
	// }
	acao="atu";
	...
	PedidoSaidaSub pedidoSaida = daoPedidoSaida.procurarPedidoSaida(Integer.parseInt(idPedidoSaida));
	idLoja = "".valueOf(pedidoSaida.getIdLoja());
	//idMesa = "".valueOf(daoPedidoSaida.);
	...
	if(pedidoSaida.getPessoaResponsavel()!= null){
	...
	quantPessoas = "".valueOf(daoPedidoSaida.getQuantPessoas(pedidoSaida.getIdPedidoSaida()));
	acao="atu";
}
%>
<body id="editar" onload="document.forms[0].elements[2].focus();" >
<% if (mensagem != null) { %><div class="mensagem"><center><%= mensagem %></center></div><hr><% } %>
<form method="post" action="editarFormPedido.jsp">
...
<!-- 
  <tr>
   ...
   -->
    <tr>
      <th class="label">Garçom*</th>
      <td><select name="idColaborador">
          <option value='0'>Selecione...</option>
          <%@include file="../../WEB-INF/jspf/combo/comboGarcom.jspf"%></select>
          <script>comboSelect(document.forms[0].idColaborador, '<%= idColaborador %>');</script>
      </td>
  </tr>
  <tr>
      <th class="label">Mesa</th>
      <td><select disabled name="idMesa">
          <option value='0'>Selecione...</option>
          <%@include file="../../WEB-INF/jspf/combo/comboMesa.jspf"%></select>
          <script>comboSelect(document.forms[0].idMesa, '<%=idMesa%>');</script>
      </td>
  ...
  <tr>
    <th class="label">Quant. Pessoas</th>
    <td><input type="text" name="quantPessoas" <%if (quantPessoas != null) { %>value="<%=quantPessoas%>"<% }%> size="60" maxlength="60"></td>
  </tr>
  <!-- 
    <tr>
...
<table>
   ...
   <tr>
      <th class="label">Transf. Mesa</th>
      <td><select name="idMesaTroca"> 
          <option value='0'>Selecione...</option>
          <%@include file="../../WEB-INF/jspf/combo/comboMesaTroca.jspf"%></select>
          <script>comboSelect(document.forms[0].idMesaTroca, '<%= idMesaTroca%>');</script>
      </td>
      <td><input class="button" type="button" value="Transferir" onClick="javascript: trocar('<%="transf"%>');" /></td>
  </tr>
</table>
 <hr>
