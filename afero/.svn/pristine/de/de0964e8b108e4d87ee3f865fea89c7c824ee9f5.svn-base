...
clausula = clausula+" AND ps.tipoPedido='S' AND ps.operacao='S' AND ps.idLoja = "+Integer.parseInt(idLojaUsuario);
...
for ( Iterator it = list.iterator(); hasNext && cont < gridSize; hasNext = it.hasNext()) {
	...
	if(pedSaida.getStatusPag() != null){
		statusPagamento = pedSaida.getStatusPag();
	}else{
		statusPagamento = "N";
	}
...
%>
   ...
   <tr>
      ...
      <%if(pedSaida.getStatus().equalsIgnoreCase("A")){%>
      	<td class = "grid" width="2%"><center><a target="_blank" href="ordemProd.jsp?idPedidoSaida=<%=pedSaida.getIdPedidoSaida()%>"><img border = "0" src="../images/ficha.gif"></a></center></td>
      <%}else{%>
      ...
      <%if((pedSaida.getStatus().equalsIgnoreCase("P")|| pedSaida.getStatus().equalsIgnoreCase("F"))&& statusPagamento.equalsIgnoreCase("N")){%>
