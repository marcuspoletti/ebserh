...
if(idVendedor.equalsIgnoreCase("0")){
	clausula = " WHERE ps.tipoPedido in ('T') AND ps.status='A' AND ps.dtPag BETWEEN '"+converte.DMYToYMDI(dataIni)+" "+horaIni+"' AND '"+converte.DMYToYMDI(dataFim)+" "+horaFim+"'";
}else{
	clausula = " WHERE ps.tipoPedido in ('T') AND ps.idVendedor="+Integer.parseInt(idVendedor)+" AND ps.status='A' AND ps.dtPag BETWEEN '"+converte.DMYToYMDI(dataIni)+" "+horaIni+"' AND '"+converte.DMYToYMDI(dataFim)+" "+horaFim+"'";
}
