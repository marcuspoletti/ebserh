...
<script>
	function Salvar() {
		...
		} else if (document.all.correntista.value == "") {
			window.alert("O campo Correntista é obrigatório.");
			document.all.correntista.focus();
		} else if (document.all.dtReceb.value == ''	&& document.all.status[1].checked == true) {
			window.alert("O campo Data de Compesação deve ser informado.");
			document.all.dtReceb.focus();
		} else {
...
if (acao.equalsIgnoreCase("atu")) {
  ...
  ChegueRecebido chequeRecebido = dao.procurarChegueRecebido(Integer.parseInt(idChequeRecebido));
...
<table border="0" width="100%">
  ...
  <tr>
  	<th class="label">Conta Corrente/DV</th>
  ...
  <tr>
  	...
  	<td>
  		<textarea name="observacao" cols="60" rows="2" maxlength="200" ><%if (observacao != null) {%> <%=observacao%> <% }%></textarea>
  	</td>
  </tr>
  <tr>
