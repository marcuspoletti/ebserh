...
<script>
function Salvar() {
	  if (document.all.dsEntidade.value == "") {
	  ...
	  } else if (document.all.nrDiasPag.value == '') {
          	    window.alert("O campo Dias para Pagamento é obrigatório.");
                      document.all.nrDiasPag.focus();
	  } else if (document.all.valorTotal.value == '0,00') {
    	    	    window.alert("O campo Valor Total é obrigatório.");
...
function recarregar(acao) {
  document.forms[0].action='formDuplicataReceber.jsp?acao='+acao;
  document.forms[0].submit();
}
function Cancelar() {
  document.forms[0].action='formDuplicataReceber.jsp';
  document.forms[0].submit();
}
...
if (quantParcelas == null)quantParcelas="1";
   ...
   <tr>
    ...
    <tr>
      <th class="label" style="height: 22px">Receber de*</th>
            <td style="height: 15px"><input disabled type="text" name="dsEntidade" <%if (dsEntidade != null) { %>value="<%=dsEntidade%>"<% }%>  size="60" maxlength="60"> <a  href="consultarClientesContasReceber.jsp?acao=<%=acao%>&cdEntidade=<%=cdEntidade%>&dc=<%=dc%>&dtEmissao=<%=dtEmissao%>&dtComp=<%=dtComp%>&nrDoc=<%=nrDoc%>&idTipoDocumento=<%=idTipoDocumento%>&idPlanoConta=<%=idPlanoConta%>&
