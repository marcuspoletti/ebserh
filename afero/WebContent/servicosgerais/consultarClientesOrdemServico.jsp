...
if(cpfLocalizar == null)cpfLocalizar="";
  ...
  if (!cpfLocalizar.equals("")) {
    if (clausula.isEmpty()) {
    	...
    	if(tpInsc.equalsIgnoreCase("F")){
    		...
    		clausula += " AND j.cnpj LIKE '"+cpfLocalizar+"%'";
    	}
  ...
  if (!statusLocalizar.equals("T")) {
    if (clausula.isEmpty()) {
      ...
      clausula += " AND e.status='"+statusLocalizar+"'";
    }
  }
...
<form action="consultarClientesOrdemServico.jsp">
...
<input type="hidden" name="dtOS" value="<%=dtOS %>">
<input type="hidden" name="dtAprov" value="<%=dtAprov %>">
<input type="hidden" name="dtEntrega" value="<%=dtEntrega %>">
<input type="hidden" name="pessoaResponsavel" value="<%=pessoaResponsavel %>">
<input type="hidden" name="dtInicio" value="<%=dtInicio %>">
<input type="hidden" name="dtConclusao" value="<%=dtConclusao %>">
<input type="hidden" name="dtEntRealizada" value="<%=dtEntRealizada %>">
...
<input type="hidden" name="cmpOS1" value="<%=cmpOS1 %>">
<input type="hidden" name="cmpOS2" value="<%=cmpOS2 %>">
<input type="hidden" name="cmpOS3" value="<%=cmpOS3 %>">
<input type="hidden" name="cmpOS4" value="<%=cmpOS4 %>">
<input type="hidden" name="observacao" value="<%=observacao %>">
<input type="hidden" name="vlServ" value="<%=vlServ %>">
<input type="hidden" name="vlProd" value="<%=vlProd %>">
<input type="hidden" name="vlFrete" value="<%=vlFrete %>">
<input type="hidden" name="vlDesc" value="<%=vlDesc %>">
...
%>
    ...
    <tr>
    	...
    	<td class = "grid"><center><a class="link" target="principal" href="formOrdemServico.jsp?acao=<%=acao%>&idOrdemServico=<%=idOrdemServico%>&idLoja=<%=idLoja%>&idColaborador=<%=idColaborador%>&idEntrega=<%=idEntrega%>&cdTipoAtendimento=<%=cdTipoAtendimento%>&tipo=<%=tipo%>
    	&dtOrc<%=dtOrc%>&dtOS=<%=dtOS%>&dtAprov=<%=dtAprov%>&dtEntrega=<%=dtEntrega%>&pessoaResponsavel=<%=pessoaResponsavel%>&dtInicio=<%=dtInicio%>&dtConclusao=<%=dtConclusao%>&dtEntRealizada=<%=dtEntRealizada%>&cdCondPagto=<%=cdCondPagto%>
    	&prazoEntrega=<%=prazoEntrega%>&prazoValidade=<%=prazoValidade%>&prazoGarantia=<%=prazoGarantia%>&cmpOS1=<%=cmpOS1%>&cmpOS2=<%=cmpOS2%>&cmpOS3=<%=cmpOS3%>&cmpOS4=<%=cmpOS4%>&vlServ=<%=vlServ%>&vlProd=<%=vlProd%>&vlFrete=<%=vlFrete%>
    	&vlDesc=<%=vlDesc%>&status=<%=status%>&dtMod=<%=dtMod%>&dsEntidade=<%=nomeEntidade %>&cdEntidade=<%=cdEntidade %>" onclick="javascript:window.close()"><%=nomeEntidade%></a></center></td>
