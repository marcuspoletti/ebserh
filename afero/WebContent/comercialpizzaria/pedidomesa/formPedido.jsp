...
function salvar(acao) {
  if (document.all.cdEntidade.value == '0') {
...
if(quantPessoas == null)quantPessoas = "1";
//MESA
...
String idColaborador = "".valueOf(colaborador.getIdColaborador());
...
double vlPed = 0;
String observacao = "";
String anotacao = referencia;
String pessoaResponsavel = "";
String status = "P";
String operacao = "S";
String statusPag = "N";
String tipoPedido = "PM";
String dtMod = "";
String usuario = (String)session.getAttribute("Login");
String dtEntrega = daoPedidoSaida.dataAtualPedido();
String hrEntrega = "00:00";
if (idPedidoSaida == null || idPedidoSaida.equals("0")) {
	idPedidoSaida = "0";
	acao="inc";
} else if (!idPedidoSaida.equals("0")) {
	...
	usuario = pedidoSaida.getUsuario();
	...
	acao="atu";
	PedidoSaidaDAO daoPedidoSaidaAtu = new PedidoSaidaDAO(conn);
	daoPedidoSaidaAtu.atualizar(pedidoSaida);
}
%>
...
<input type="hidden" name="statusPag" value="<%=statusPag%>"/>
...
<table border="0" width="100%">
  ...
  <tr>
   ...
   --%>
    ...
    <tr>
      ...
      <td><select disabled name="idMesa" onchange="recarregar();">
          <option value='0'>Selecione...</option>
    ...
    <tr>
      <th class="label" style="height: 22px">Cliente*</th>
          <td style="height: 15px"><input disabled type="text" name="dsEntidade" <%if (dsEntidade != null) { %>value="<%=dsEntidade%>"<% }%>  size="60" maxlength="60"> <a href="consultarClientes.jsp?acao=<%=acao%>&idPedidoSaida=<%=idPedidoSaida%>&idLoja=<%=idLoja%>&idEntrega=<%=idEntrega%>&idColaborador=<%=idColaborador%>&cdCondPagto=<%=cdCondPagto%>&idMesa=<%=idMesa%>"><img border = "0" src="../../images/pesquisa.gif"></a></td>
  </tr>
  <tr>
  ...
  <tr>
   ...
   <tr>
    ...
    <td><input disabled type="text" name="dtOrc" <%if (dtPed != null) { %>value="<%=dtPed%>"<% }%>  size="10" maxlength="10"></td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td class="label_radio">
      ...
      <input disabled type="radio" class="radio" name="status" value="C" <%= (status.equals("C")? "checked": "") %>>Cancelado
    </td>
  </tr>
  <%--
  <tr>
    ...
    <td class="label_radio">
      ...
      <input disabled type="radio" class="radio" name="operacao" value="ES" <%= (operacao.equals("ES")? "checked": "") %>>Estorno de Entrada Por Devolução
