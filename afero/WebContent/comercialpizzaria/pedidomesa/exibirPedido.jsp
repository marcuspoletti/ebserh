...
if(request.getParameter("valorEntrega") != null){
	...
	pedidoSaida.setVlFrete(valorEntrega);
	daoPedidoSaida.atualizarVlFrete(Integer.parseInt(idPedidoSaida), valorEntrega);
}else{
	pedidoSaida.setVlFrete(10);
	valorEntrega = pedidoSaida.getVlFrete();
}
double valorTotal = 0;
float desconto = 0;
...
if(request.getParameter("desconto") != null){
	...
	desconto = Utilitaria.toNumber(request.getParameter("desconto")).floatValue();
	pedidoSaida.setVlDesc(desconto);
	daoPedidoSaida.atualizar(pedidoSaida);
}else{
	desconto = pedidoSaida.getVlDesc();
}
if(request.getParameter("valorTotal") != null){
	...
	valorTotal = pedidoSaida.getVlPed();
...
/* if(request.getParameter("valorTotalDesconto") != null){
	valorTotalDesconto = pedidoSaida.getVlPed();
}else{
	valorTotalDesconto = pedidoSaida.getVlPed();
} */
...
if(request.getParameter("total") != null){
	totalCarga = Utilitaria.toNumber(request.getParameter("total")).doubleValue();
   if(desconto > 0){
	   calcDesc = (pedidoSaida.getVlDesc()*totalCarga)/100;
	   valorTotal = totalCarga - calcDesc;
	   valorTotalDesconto = valorTotal;
	   pedidoSaida.setVlPed(valorTotal);
	   daoPedidoSaida.atualizar(pedidoSaida);
	   totalCarga = valorTotal;
   }else{
	   valorTotal = pedidoSaida.getVlPed();
	   valorTotalDesconto = pedidoSaida.getVlPed();
	   daoPedidoSaida.atualizar(pedidoSaida);
   ...
   if(valorEntrega > 0){
	   calcTaxa = (pedidoSaida.getVlFrete()*totalCarga)/100;
	   valorTotal = totalCarga + calcTaxa;
	   pedidoSaida.setVlPed(valorTotal);
	   daoPedidoSaida.atualizar(pedidoSaida);
   }else{
	   valorTotal = pedidoSaida.getVlPed();
	...
	if(valorEntrega > 0){
		   calcTaxa = (valorEntrega*pedidoSaida.getVlPed())/100;
		   valorTotal = pedidoSaida.getVlPed() + calcTaxa;
		   pedidoSaida.setVlPed(valorTotal);
		   daoPedidoSaida.atualizar(pedidoSaida);
	 }
...
String observacao = "";
if(pedidoSaida.getObservacao() != null){
		...
		if (!pedidoSaidaUpdate.getStatus().equals("A")) {
			...
			if(status.equalsIgnoreCase("A") || status.equalsIgnoreCase("F")){
				...
				daoMesa = new MesaDAO(conn);
				 int isdMesa = daoMesa.getIdMesa(Integer.parseInt(idPedidoSaida));
				 if(isdMesa != 0){
					 daoMesa.atualizarStatus(isdMesa, "L"); 
				 }
				statusPag = "P";
				daoPedidoSaidaUpdate.atualizarStatusPag(Integer.parseInt(idPedidoSaida),statusPag);
				...
				if(status.equalsIgnoreCase("CA")){
					 MesaDAO daoMesaCancelado = new MesaDAO(conn);
					 int isdMesa = daoMesaCancelado.getIdMesa(Integer.parseInt(idPedidoSaida));
					 if(isdMesa != 0){
						 daoMesaCancelado.atualizarStatus(isdMesa, "L"); 
					 }
					statusPag = "N";
					daoPedidoSaidaUpdate.atualizarStatusPag(Integer.parseInt(idPedidoSaida),statusPag);
...
<table border="0" width="70%">
  ...
  <tr>
    ...
    <td class="label_radio">
      ...
      <input type="radio" class="radio" name="status" value="CA" <%= (status.equals("CA")? "checked": "") %>>Cancelado
    </td>
  <tr>
...
<% 
 ...
 /* if(request.getParameter("total") == null){
	 if(valorEntrega > 0){
		   calcTaxa = (valorEntrega*total)/100;
		   valorTotal = total + calcTaxa;
		   pedidoSaida.setVlPed(valorTotal);
		   daoPedidoSaida.atualizar(pedidoSaida);
	 }
...
%> 
  <tr>
      ...
      <th class="grid" width="5%"><center>TOTAL DO PED.:</center></th>
      <td><input type="text" id="total" name="total" value="<%=Utilitaria.formatarNumero(total,2)%>" ></td>
  </tr>
     <tr>
      
      <th class="grid" width="5%"><center>DESC. :(%)</center></th>
      <td><input type="text" id="desconto" name="desconto" value="<%=Utilitaria.formatarNumero(desconto, 0)%>" onblur="salvar()"></td>
  </tr> 
   <tr>
      
      <th class="grid" width="5%"><center>VL. C/ O DESC. :</center></th>
      <td><input type="text" id="valorTotalDesconto" name="valorTotalDesconto" value="<%=Utilitaria.formatarNumero(valorTotalDesconto, 2)%>" ></td>
  </tr>
  
   <tr>
      
      <th class="grid" width="5%"><center>TX. DE SERV. (%):</center></th>
      <td><input type="text" id="valorEntrega" name="valorEntrega" value="<%=Utilitaria.formatarNumero(valorEntrega, 0)%>"  onblur="salvar()"></td>
  </tr> 
...
<tr>
	<td><input class="button" type="button" value="Gravar" onClick="javascript: salvar();" /> 
	<input class="button" type="button" value="Sair" onClick="javascript:window.close()" /> 
</td>
</tr>
</table>
<br>
