...
<%
				...
				for (Iterator itListarSubItens = listarEstoqueSubItens.iterator(); itListarSubItens.hasNext();) {
					...
					if (precoListarSubList != null) {
						...
						if (pedidoSubItens.getDsCompProduto() == null) {
							...
							descricao = Utilitaria.abreviarStr(prodSubItens.getDsProduto(),	28).trim() + " " +
													Utilitaria.abreviarStr(pedidoSubItens.getDsCompProduto(),	28).trim();
						}
						if (descricao.trim().length()>28) {
							textoBobina.append(cb(descricao.substring(0,28), 29)
									+ cb(Utilitaria.formatarNumero(pedidoSubItens.getQuant(),	0), 6)
	...
	if (!subItemOk) {
		...
		if (descricao.trim().length()>28) {
			textoBobina.append(cb(descricao.substring(0,28), 29)
				+ cb(Utilitaria.formatarNumero(pedidoSaidaItem.getQuant(),	0), 6)
				+ cb(Utilitaria.formatarNumero(precoItem, 2), 10)
				+ "\n");
			textoBobina.append(cb(descricao.substring(28), 29)
				+ "\n");
