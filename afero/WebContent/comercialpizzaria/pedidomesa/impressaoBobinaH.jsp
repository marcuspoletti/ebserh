...
<%
	...
	int idMesa = daoMesa.getIdMesa(Integer.parseInt(idPedidoSaida));
	Mesa mesa = daoMesa.procurarMesa(idMesa);
	...
	float valorEntrega = daoPedidoSaidaSub.mostrarVlFrete(Integer.parseInt(idPedidoSaida));
	...
	double valorTotalDesc = pedidoSaida.getVlPed();
	...
	textoBobina.append("========================================\n");
	textoBobina.append("Mesa.......: ").append((String)mesa.getDsMesa()).append("\n");
	...
	textoBobina.append("========================================\n");
	textoBobina.append("PRODUTO                     QTD       R$\n");
	textoBobina.append("========================================\n");
				...
				for (Iterator itListarSubItens = listarEstoqueSubItens.iterator(); itListarSubItens.hasNext();) {
					...
					if (precoListarSubList != null) {
						...
						if (pedidoSubItens.getDsCompProduto() == null) {
							descricao = Utilitaria.abreviarStr(prodSubItens.getDsProduto(),	28).trim();
						} else {
							descricao = Utilitaria.abreviarStr(prodSubItens.getDsProduto(),	28).trim() + " " +
													Utilitaria.abreviarStr(pedidoSubItens.getDsCompProduto(),	35).trim();
						}
						if (descricao.trim().length()>28) {
							textoBobina.append(cb(descricao.substring(0,28), 29)
									+ cb(Utilitaria.formatarNumero(pedidoSubItens.getQuant(),	0), 7)
									+ "\n");
							textoBobina.append(cb(descricao.substring(28), 29)
									+ "\n");
						} else {
							textoBobina.append(cb(descricao, 28)
									+ cb(Utilitaria.formatarNumero(pedidoSubItens.getQuant(),	0), 7)
									+ "\n");
						}
						subItemOk = true;
	...
	if (!subItemOk) {
		...
		if (pedidoSaidaItem.getDsCompProduto() == null) {
			...
			descricao = Utilitaria.abreviarStr(prodSubItens.getDsProduto(),	28).trim() + " " +
									Utilitaria.abreviarStr(pedidoSaidaItem.getDsCompProduto(),	28).trim();
		}
		if (descricao.trim().length()>28) {
			textoBobina.append(cb(descricao.substring(0,28), 29)
				+ cb(Utilitaria.formatarNumero(pedidoSaidaItem.getQuant(),	0), 7)
				+ cb(Utilitaria.formatarNumero(precoItem, 2), 10)
				+ "\n");
			textoBobina.append(cb(descricao.substring(28), 28)
				+ "\n");
		} else {
			textoBobina.append(cb(descricao, 28)
				+ cb(Utilitaria.formatarNumero(pedidoSaidaItem.getQuant(),	0), 7)
	...
	textoBobina.append("QUANT. PESSOAS     :-->" + daoPedidoSaidaSub.getQuantPessoas(Integer.parseInt(idPedidoSaida))).append("\n");
	textoBobina.append("MEDIA P/ PESSOAS   :-->" + Utilitaria.formatarNumero((valorTotal/daoPedidoSaidaSub.getQuantPessoas(Integer.parseInt(idPedidoSaida))), 2)).append("\n");
	...
	if(desconto > 0){
		double calcDesc = ((total * desconto)/100);
		valorTotalDesc = (total - calcDesc);
	}else{
		valorTotalDesc = pedidoSaida.getVlPed();
	}
	textoBobina.append("VALOR C/ DESC.(R$) :-->" + Utilitaria.formatarNumero(valorTotalDesc, 2)).append("\n");
	textoBobina.append("TAXA DE SERVIÇO :-->" + Utilitaria.formatarNumero(((total*valorEntrega)/100), 2)).append("\n");
	...
	//textoBobina.append("----------------(Corte)-----------------\n");
	//textoBobina.append("\n");
	
	//textoBobina.append("      Pedido de Venda N. ").append("00" + idPedidoSaida).append("\n");
	//textoBobina.append("========================================\n");
	//textoBobina.append("PRODUTO                             QTD.\n");
	//textoBobina.append("========================================\n");
	list = daoPedidoSaidaItem.procurarPedidoSaidaItem(Integer.parseInt(idPedidoSaida));
	if (list != null) {
		for (Iterator it = list.iterator(); it.hasNext();) {
	pedidoSaidaItem = (PedidoSaidaSubItem) it.next();
	String impProducao = daoPedidoSaidaItem.getImpProducao(pedidoSaidaItem.getIdPedidoSaidaItem());
	if (impProducao != null && impProducao.equalsIgnoreCase("S")) {
		listSubItens = daoPedidoSaidaSubItens.listarPedidoSaidaSubItensListCompProduto(pedidoSaidaItem.getIdPedidoSaidaItem());
		subItemOk = false;
		if (listSubItens != null) {
			for (Iterator itSubItens = listSubItens.iterator(); itSubItens.hasNext();) {
				...
				if (listarEstoqueSubItens != null) {
					for (Iterator itListarSubItens = listarEstoqueSubItens.iterator(); itListarSubItens.hasNext();) {
						...
						if (precoListarSubList != null) {
							...
							if (pedidoSubItens.getDsCompProduto() == null) {
								descricao = Utilitaria.abreviarStr(prodSubItens.getDsProduto(),	35).trim();
							} else {
								descricao = Utilitaria.abreviarStr(prodSubItens.getDsProduto(),	35).trim() + " " +
														Utilitaria.abreviarStr(pedidoSubItens.getDsCompProduto(),	35).trim();
							}
							if (descricao.trim().length()>36) {
								//textoBobina.append(cb(descricao.substring(0,35), 36)
								//		+ (pedidoSubItens.getQuant() == 1 ? "*" : cb(Utilitaria.formatarNumero(pedidoSubItens.getQuant(),	0), 4))
								//		+ "\n");
								//textoBobina.append(cb(descricao.substring(36), 36)
								//		+ "\n");
							} else {
								//textoBobina.append(cb(descricao, 36)
								//		+ (pedidoSubItens.getQuant() == 1 ? "*" : cb(Utilitaria.formatarNumero(pedidoSubItens.getQuant(),	0), 4))
								//		+ "\n");
							}
							subItemOk = true;
			...
			if (!subItemOk) {
				...
				prodSubItens = daoProdutoSubItens.procurarProduto(pedidoSaidaItem.getIdProduto());
				//textoBobina.append(cb(Utilitaria.abreviarStr(prodSubItens.getDsProduto(),	35), 36)
				//		+ (pedidoSaidaItem.getQuant() == 1 ? "*" : cb(Utilitaria.formatarNumero(pedidoSaidaItem.getQuant(),	0), 4))
				//		+ "\n");
				if (pedidoSaidaItem.getDsCompProduto() == null) {
					descricao = Utilitaria.abreviarStr(prodSubItens.getDsProduto(),	35).trim();
				} else {
					descricao = Utilitaria.abreviarStr(prodSubItens.getDsProduto(),	35).trim() + " " +
											Utilitaria.abreviarStr(pedidoSaidaItem.getDsCompProduto(),	35).trim();
				}
				if (descricao.trim().length()>36) {
					//textoBobina.append(cb(descricao.substring(0,35), 36)
					//	+ (pedidoSaidaItem.getQuant() == 1 ? "*" : cb(Utilitaria.formatarNumero(pedidoSaidaItem.getQuant(),	0), 4))
