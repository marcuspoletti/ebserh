...
for ( Iterator it = list.iterator(); hasNext && cont < gridSize; hasNext = it.hasNext()) {
	...
	if(tpInsc.equalsIgnoreCase("F")){
		...
		cdEntidade = entidadeF.getCdEntidade();
...
%>
    <tr>
      ...
      <td class = "grid"><center><a class="link" target="principal" href="formPedido.jsp?cdCondPagto=<%=cdCondPagto%>&cdEntidade=<%=cdEntidadePedido%>&acao=<%=acao%>&dsEntidade=<%=entidadeFisica.getNome()%>&idLoja=<%=idLoja%>&idEntrega=<%=idEntrega%>&idColaborador=<%=idColaborador%>"onclick="javascript:window.close()"><%=entidadeFisica.getNome()%></a></center></td>
