...
if (acao.equalsIgnoreCase("inc") || acao.equalsIgnoreCase("atu") || acao.equalsIgnoreCase("exc")) {
  ...
  impProducao = request.getParameter("impProducao");
  quantFatias = request.getParameter("quantFatias");
  if(impProducao == null)impProducao = "N";
  if(quantFatias == null)quantFatias = "0";
  //Comandas
	...
	try {
	  ...
	  if (!acao.equalsIgnoreCase("exc")){
		  ...
		  produto.setIdGrupo(idGrupo);  
           ...
           if(Utilitaria.toNumber(quantFatias).intValue() != 0){
        	     produto.setQuantFatias(Utilitaria.toNumber(quantFatias).intValue());
           }else{
        	     produto.setQuantFatias(0);
           }
           if(impProducao != null){
        	     produto.setImpProducao(impProducao);
           }else{
        	   produto.setImpProducao("N");
           }
  ...
  if (acao.equalsIgnoreCase("inc")) {
	  //inclui no banco de dados
  	         ...
  	         dao.incluirImpProducaoQuantFatias(produto, cdProduto);//Comandas
...
for ( Iterator it = list.iterator(); hasNext && cont < gridSize; hasNext = it.hasNext()) {
	...
	impressaoProducao = "NÃO";
	if(dao.getImpProducao(prod.getIdProduto()) != null){
		if(dao.getImpProducao(prod.getIdProduto()).equalsIgnoreCase("S")){
			impressaoProducao = "SIM";
		}
	}
...
%>
    <tr>
      ...
      <td class = "grid" width="5%"><center><%=impressaoProducao%></center></td>
...
<p align="right">|<a class="button" href="formProduto.jsp?acao=inc">Novo Cadastro</a>|
<%  if (rowNum != 0 || hasNext) { %>
        <center>
<%      if (rowNum != 0) { %>
