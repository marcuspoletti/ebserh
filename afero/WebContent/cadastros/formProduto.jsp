...
function pesquisarNcm() {
	var str;
	str = document.getElementById("cdNcm").value;
	if (str == "" && str == "0") {
		document.getElementById("cdNcm").value = "0";
		return;
	}
	loadXMLDoc(
			 "pesquisaCdNcm.jsp?cdNcm=" + str,
			function() {
				var retorno;
				var resposta = [];
				if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
					...
					var msg = resposta[2].toString();
					if (msg=="OK") {
						document.getElementById("cdNcm").value = resposta[1];
...
function pesquisarCest() {
	var str;
	str = document.getElementById("cdCest").value;
	if (str == "" && str == "0") {
		document.getElementById("cdCest").value = "0";
		return;
	}
	loadXMLDoc(
			 "pesquisaCest.jsp?cdCest=" + str,
			function() {
				var retorno;
				var resposta = [];
				if (xmlHttp.readyState == 4 && xmlHttp.status == 200) {
					...
					var msg = resposta[2].toString();
					if (msg=="OK") {
						document.getElementById("cdCest").value = resposta[1];
...
if (acao.equalsIgnoreCase("atu")){
	    ...
	    if(cdDivisao != null){
			...
			cdCatTributaria = "".valueOf(dao.procurarIdCatTributaria(Integer.parseInt(idProduto)));
			cdNcm = dao.procurarCdNcm(Integer.parseInt(idProduto));
			descricaoNcm = dao.procurarAutoCompleteCdNcm(Integer.parseInt(idProduto));
			cdCest = dao.procurarCdCest(Integer.parseInt(idProduto));
			descricaoCest = dao.procurarAutoCompleteCdCeset(Integer.parseInt(idProduto));
			cdCfopSaidaDentro = "".valueOf(dao.procurarCfopSaidaDentro(Integer.parseInt(idProduto)));
			cdCfopSaidaFora = "".valueOf(dao.procurarCfopSaidaFora(Integer.parseInt(idProduto)));
			cdBarraTrib = dao.procurarCdBarraTrib(Integer.parseInt(idProduto));
			cdExTipi = dao.procurarExTipi(Integer.parseInt(idProduto));
			cdGenero = dao.procurarCdGenero(Integer.parseInt(idProduto));
...
%>
<body onload="DefinirAbas(<%=contAbas%>); AlternarAbas(<%=selectAba%>,<%=selectAba%>, <%=contAbas%>); document.forms[0].elements[2].focus(); " >
...
<%if(acao.equalsIgnoreCase("atu")) { %>
						...
						<td height="50" width="120" class="abas-menu" id="td_aba4"
							onClick="AlternarAbas('4','4',<%=contAbas%>)"><font size=2>Fiscal
						</td>
...
<%} %>
					...
					<%} %>
						  ...
						  <tr>
     							    ...
     							    <td><select id="idDivisao" name="idDivisao" onchange="recarregar('<%=acao%>');" required="true">
                        ...
                        <tr>
    							...
    							<td><input type="text" id="nmProduto" name="nmProduto" value="<%=nmProduto%>"  size="100" maxlength="100"></td>
  						</tr>

  						<tr>
    							<th class="label">Descrição Produto*</th>
    							<td><input type="text" id="dsProduto" name="dsProduto" value="<%=dsProduto%>"  size="100" maxlength="100"></td>
  						</tr>
  						<tr>
						...
						<table>
						<tr>
    							<th class="label">Código do Genero</th>
    							<td><input type="text" id="cdGenero" name="cdGenero" <%if (cdGenero != null) { %>value="<%=cdGenero%>"<% }%>  size="2" maxlength="2"></td>
  					    	</tr>
  					    	<tr>
    							<th class="label">Código Tx Tipi</th>
    							<td><input type="text" id="cdExTipi" name="cdExTipi" <%if (cdExTipi != null) { %>value="<%=cdExTipi%>"<% }%>  size="3" maxlength="3"></td>
  					    	</tr>
						 <tr>
						    <th class="label">Cat. Tributária</th>
						    <td><select name="cdCatTributaria" >
						        <option value='0'>Selecione...</option>
						        <%@include file="../WEB-INF/jspf/combo/comboCatTributaria.jspf" %></select>
						        <script>comboSelect(document.forms[0].cdCatTributaria, '<%= cdCatTributaria%>');</script>
						    </td>
						  </tr>
						  <tr>
						    <th class="label">Ncm</th>
						    <td>
						        <input name="cdNcm" id="cdNcm" <%if (descricaoNcm != null) { %>value="<%=descricaoNcm%>"<% }%>size="60" maxlength="250">
						        <script>
        							$("#cdNcm").autocomplete("pesquisaCdNcm.jsp");
   						        </script>
					          ...
					          <tr>
						    <th class="label">Cest</th>
						    <td>
						        <input name="cdCest" id="cdCest" <%if (descricaoCest != null) { %>value="<%=descricaoCest%>"<% }%>size="60" maxlength="250">
						        <script>
        							$("#cdCest").autocomplete("pesquisaCest.jsp");
   						        </script>
						    </td>
					          </tr>
						  <tr>
						    <th class="label">CFOP Saída Dentro UF</th>
						    <td><select name="cdCfopSaidaDentro">
						        <option value='0'>Selecione...</option>
						        <%@include file="../WEB-INF/jspf/combo/comboCfopDentroUf.jspf" %></select>
						        <script>comboSelect(document.forms[0].cdCfopSaidaDentro, '<%=cdCfopSaidaDentro%>');</script>
						    </td>
						  </tr>
						  <tr>
						    <th class="label">CFOP Saida Fora UF</th>
						    <td><select name="cdCfopSaidaFora">
						        <option value='0'>Selecione...</option>
						        <%@include file="../WEB-INF/jspf/combo/comboCfopSaida.jspf" %></select>
						        <script>comboSelect(document.forms[0].cdCfopSaidaFora, '<%=cdCfopSaidaFora%>');</script>
						    </td>
						  </tr>
						  <tr>
    							<th class="label">Código de Barras Trib</th>
    							<td><input type="text" id="cdBarraTrib" name="cdBarraTrib" <%if (cdBarraTrib != null) { %>value="<%=cdBarraTrib%>"<% }%>  size="60" maxlength="60"></td>
  					    	</tr>
						</table>
						</div>
