...
function cancelar() {
  document.forms[0].action="formEntidadeJuridicaSimplificado.jsp";
  document.forms[0].submit();
}

function voltar() {
  document.forms[0].action = 'listarEntidadeJuridicaSimplificado.jsp?acao=voltar'
	document.forms[0].submit();
}
...
<%
	//variáveis capturadas da página listarEntidadeJuridicaSimplificado.jsp
...
<form method="post" action="listarEntidadeJuridicaSimplificado.jsp?acao=<%=acao%>&cdEntidade=<%=cdEntidade%>">
...
<%if(acao.equalsIgnoreCase("atu")) { %>
						...
						<td height="50" width="120" class="abas-menu" id="td_aba2"
							onClick="AlternarAbas('2','2',<%=contAbas%>);"><font size=2>Contatos
						</td>
<%} %>
...
<%} %>
						...
						<table width="98%">
							<tr>
								<td><br>|<a class="button" href="listarEntidadeContatoSimplificado.jsp?cdEntidade=<%=cdEntidade%>&acao=listar&selectAba=<%=selectAba%>&acaoAnterior=<%=acao%>">Cadastro Contato</a>|<br></td>
							</tr>
						</table>
						</div>
