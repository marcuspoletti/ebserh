...
function cancelar() {
  document.forms[0].action="formEntidadeJuridica.jsp";
  document.forms[0].submit();
}

function voltar() {
  document.forms[0].action = 'listarEntidadeJuridica.jsp?acao=voltar'
	document.forms[0].submit();
}
...
<%
	//variáveis capturadas da página listarEntidadeJuridica.jsp
  ...
  String mensagem = request.getParameter("mensagem");
	...
	if (acao.equalsIgnoreCase("atu")) {
		...
		EntidadeJuridicaDAO dao1 = new EntidadeJuridicaDAO(conn);
		EntidadeJuridica entidadeJuridica = dao1.procurarEntidadeJuridica(Integer.parseInt(cdEntidade));
		cpfCnpj = entidadeJuridica.getCnpj(); 
	  razaoSocial = entidadeJuridica.getRazaoSocial();
	  dtAbertura = ConverteDate.dateToStr(entidadeJuridica.getDtAbertura(),"dd/MM/yyyy");
	  if (dtAbertura == null) dtAbertura = "";
	  inscEstadual = entidadeJuridica.getInscEstadual();
	  codCnae = entidadeJuridica.getCodCnae();
	...
	RamoAtividadeDAO dao6 = new RamoAtividadeDAO(conn);
	List listRamoAtividade = dao6.listarRamoAtividade("");

%>
<body onload="DefinirAbas(<%=contAbas%>); AlternarAbas(<%=selectAba%>,<%=selectAba%>,<%=contAbas%>); document.forms[0].elements[4].focus(); ">
<h1 class="cabecalho_pagina">Cadastro de Entidade Jurídica</h1>
<hr><% if (mensagem != null) { %><div class="mensagem"><center><%= mensagem %></center></div><hr><% } %>
<form method="post" action="listarEntidadeJuridica.jsp?acao=<%=acao%>&cdEntidade=<%=cdEntidade%>">
				...
				<table width="100%" height="50" class="abas-tb-tabela" cellpadding="0" cellspacing="0" id="busca">
					<tr>
						...
						<td height="50" width="120" class="abas-menu" id="td_aba2"
							onClick="AlternarAbas('2','2',<%=contAbas%>);"><font size=2>Dados Complementares
						</td>
<%if(acao.equalsIgnoreCase("atu")) { %>
						<!-- exibi a alternativa td_aba3 no browser -->
						<td height="50" width="120" class="abas-menu" id="td_aba3"
							onClick="AlternarAbas('3','3',<%=contAbas%>)"><font size=2>Endereços
						</td>

						<!-- exibi a alternativa td_aba4 no browser -->
						<td height="50" width="120" class="abas-menu" id="td_aba4"
							onClick="AlternarAbas('4','4',<%=contAbas%>)"><font size=2>Telefones
						</td>

						<!-- exibi a alternativa td_aba5 no browser -->
						<td height="50" width="120" class="abas-menu" id="td_aba5"
							onClick="AlternarAbas('5','5',<%=contAbas%>)"><font size=2>Emails
						</td>

						<!-- exibi a alternativa td_aba6 no browser -->
						<td height="50" width="120" class="abas-menu" id="td_aba6"
							onClick="AlternarAbas('6','6',<%=contAbas%>)"><font size=2>Contatos
						</td>
<%} %>
...
<%} %>
						  ...
						  <tr>
								<th class="label">Tipo de Inscrição</th>
								<td class="label_radio">
								<input type="radio" class="radio" name="tpInsc" value="F" <%=(tpInsc.equals("F")? "checked": "") %> disabled="disabled">CPF 
								<input type="radio" class="radio" name="tpInsc" value="J" <%=(tpInsc.equals("J")? "checked": "") %>>CNPJ</td>
							</tr>
							<tr>
							...
							<tr>
								...
								<td><input type="text" name="site" <%if (site != null) { %>
									value="<%=site%>" <% }%> size="60" maxlength="60"></td>
							</tr>						
							<tr>
								...
								<%for ( Iterator it = listTipoEntidade.iterator(); it.hasNext(); ) {
										...
										boolean ckd = false;
										ClassificacaoDAO dao7 = new ClassificacaoDAO(conn);
										if (acao.equalsIgnoreCase("atu") && dao7.possuiClassificacao(Integer.parseInt(cdEntidade), tipoEntidade.getIdTipoEntidade())) ckd = true; %>
							...
							<tr>
								<th class="label">Áreas de atuação</th>
								<td class="label_radio">
								<%int cont = 0;
								  for ( Iterator it = listRamoAtividade.iterator(); it.hasNext(); ) {
										RamoAtividade ramoAtividade = (RamoAtividade) it.next();	
										boolean ckd = false;
										AreaAtuacaoDAO dao8 = new AreaAtuacaoDAO(conn);
										if (acao.equalsIgnoreCase("atu") && dao8.possuiAreaAtuacao(Integer.parseInt(cdEntidade), ramoAtividade.getIdRamoAtividade())) ckd = true; %>
									<input type="checkbox" class="checkbox" name="atividade" value="<%=ramoAtividade.getIdRamoAtividade() %>" <%=(ckd? "checked": "") %>> <%=ramoAtividade.getDsRamoAtividade() %>&nbsp;&nbsp;
									<%cont++;
										if (cont == 5) { %>
											<br><br>
									<%  cont = 0;
						...
						<table width="98%">
							<tr>
								...
								<td><input type="text" name="razaoSocial" <%if (razaoSocial != null) { %>
									value="<%=razaoSocial%>" <% }%> size="60" maxlength="60"></td>
							</tr>
						 	<tr>
								<th class="label">Data de abertura</th>
            		<td>
	               	<input type="text" name="dtAbertura" size="15" value="<%= dtAbertura %>" onblur="chkData(this,'dd/MM/yyyy')" maxlength="10" class="inputs">
		               	<a href="javascript:void(0)" onclick="if(self.gfPop)gfPop.fPopCalendar(document.forms[0].dtAbertura);return false;" HIDEFOCUS><img class="PopcalTrigger" align="absmiddle" src="../js/calendar/calbtn.gif" width="34" height="22" border="0" alt=""></a>
            		</td>
							</tr>
							<tr>
								<th class="label">Inscrição Estadual</th>
								<td><input type="text" name="inscEstadual" <%if (inscEstadual != null) { %>
									value="<%=inscEstadual%>" <% }%> size="15" maxlength="15"></td>
							</tr>
							<tr>
								<th class="label">Código CNAE</th>
								<td><input type="text" name="codCnae" <%if (codCnae != null) { %>
									value="<%=codCnae%>" <% }%> size="10" maxlength="10"></td>
							</tr>
						</table>
						</div>
						...
						<table>
						  ...
						  <tr>
						    <th class="label">Padrão</th>
						      ...
						      <input type="radio" class="radio" name="enderecoPadrao" disabled="disabled" value="N" <%= (enderecoPadrao.equals("N")? "checked": "") %>>Não</td>    
						  </tr>
							<tr>
								<td><br>|<a class="button" href="listarEntidadeEndereco.jsp?cdEntidade=<%=cdEntidade%>&acao=listar&selectAba=<%=selectAba%>&acaoAnterior=<%=acao%>">Cadastro Endereço</a>|<br></td>
							</tr>
						</table>
						</div>

						<!-- exibi o conteúdo div_aba4 no browser -->
						...
						<table>
						  ...
						  <tr>
						    <th class="label">Padrão</th>
						      ...
						      <input type="radio" class="radio" name="telefonePadrao" disabled="disabled" value="N" <%= (telefonePadrao.equals("N")? "checked": "") %>>Não</td>    
						  </tr>
							<tr>
								<td><br>|<a class="button" href="listarEntidadeTelefone.jsp?cdEntidade=<%=cdEntidade%>&acao=listar&selectAba=<%=selectAba%>&acaoAnterior=<%=acao%>">Cadastro Telefone</a>|<br></td>
							</tr>
						</table>
						</div>

						<!-- exibi o conteúdo div_aba5 no browser -->
						<div id="div_aba5" class="abas-conteudo" style="display: none">
						<% selectAba = "5"; %>
						<table>
						  <tr>
						  ...
						  <tr>
						    ...
						    <td><input type="text" id="dsEmail" name="dsEmail" <%if (dsEmail != null) { %>value="<%=dsEmail%>"<% }%>  size="60" maxlength="100"></td>
						  </tr>
						  <tr>
						    <th class="label">Padrão</th>
						      <td class="label_radio"><input type="radio" class="radio" name="emailPadrao" value="S" <%= (emailPadrao.equals("S")? "checked": "") %>>Sim
						      <input type="radio" class="radio" name="emailPadrao" disabled="disabled" value="N" <%= (emailPadrao.equals("N")? "checked": "") %>>Não</td>    
						  </tr>
							<tr>
								<td><br>|<a class="button" href="listarEntidadeEmail.jsp?cdEntidade=<%=cdEntidade%>&acao=listar&selectAba=<%=selectAba%>&acaoAnterior=<%=acao%>">Cadastro Email</a>|<br></td>
							</tr>
						</table>
						</div>

						<!-- exibi o conteúdo div_aba6 no browser -->
						<div id="div_aba6" class="abas-conteudo" style="display: none">
						<% selectAba = "6"; %>
						<table>
							<tr>
								<td><br>|<a class="button" href="listarEntidadeContato.jsp?cdEntidade=<%=cdEntidade%>&acao=listar&selectAba=<%=selectAba%>&acaoAnterior=<%=acao%>">Cadastro Contato</a>|<br></td>
							</tr>
						</table>
						</div>
