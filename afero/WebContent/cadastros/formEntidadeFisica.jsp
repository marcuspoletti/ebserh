...
function cancelar() {
  document.forms[0].action="formEntidadeFisica.jsp";
  document.forms[0].submit();
}
...
function cep() {
	  document.forms[0].action="formEntidadeFisica.jsp?buscaCep="+'S';
	  document.forms[0].submit();
}
...
<%
	...
	//variáveis capturadas da página listarEntidadeFisica.jsp
...
<h1 class="cabecalho_pagina">Cadastro de Entidade Física</h1>
<hr><% if (mensagem != null) { %><div class="mensagem"><center><%= mensagem %></center></div><hr><% } %>
<form method="post" action="listarEntidadeFisica.jsp?acao=<%=acao%>&cdEntidade=<%=cdEntidade%>">
...
<%if(acao.equalsIgnoreCase("atu")) { %>
						...
						<td height="50" width="120" class="abas-menu" id="td_aba6"
							onClick="AlternarAbas('6','6',<%=contAbas%>)"><font size=2>Dependentes
						</td>
<%} %>
...
<%} %>
						  ...
						  <tr>
								...
								<input type="radio" class="radio" name="tpInsc" value="F" <%=(tpInsc.equals("F")? "checked": "") %>>CPF 
								<input type="radio" class="radio" name="tpInsc" value="J" <%=(tpInsc.equals("J")? "checked": "") %> disabled="disabled">CNPJ</td>
							</tr>
							<tr>
            		...
            		<td>
	               	<input type="text" name="dtCasamento" size="15" value="<%= dtCasamento %>" onblur="chkData(this,'dd/MM/yyyy')" maxlength="10" class="inputs">
		               	<a href="javascript:void(0)" onclick="if(self.gfPop)gfPop.fPopCalendar(document.forms[0].dtCasamento);return false;" HIDEFOCUS><img class="PopcalTrigger" align="absmiddle" src="../js/calendar/calbtn.gif" width="34" height="22" border="0" alt=""></a>
							...
							<tr>
								...
								<td><input type="text" name="cargo" <%if (cargo != null) { %>
									value="<%=cargo%>" <% }%> size="40" maxlength="40"></td>
							</tr>
							<tr>
								<th class="label">Departamento</th>
								<td><input type="text" name="departamento" <%if (departamento != null) { %>
									value="<%=departamento%>" <% }%> size="40" maxlength="40"></td>
							</tr>
							<tr>
								<th class="label">Empresa</th>
								<td><input type="text" name="empresa" <%if (empresa != null) { %>
									value="<%=empresa%>" <% }%> size="40" maxlength="40"></td>
							</tr>
							<tr>
     					...
     					<tr>
				          ...
				          <script>comboSelect(document.forms[0].cdEscolaridade, '<%= cdEscolaridade %>');</script>
							...
							<tr>
								<th class="label">CPF conjuge</th>
								<td><input type="text" name="cpfConjuge" value="<%=cpfConjuge%>"
									size="14" maxlength="14" onblur="chkCPFCNPJ(this)"></td>
							</tr>
							<tr>
								<th class="label">Conjuge</th>
								<td><input type="text" name="conjuge" <%if (conjuge != null) { %>
									value="<%=conjuge%>" <% }%> size="40" maxlength="40"></td>
							</tr>
							<tr>
								<th class="label">Mãe</th>
								<td><input type="text" name="mae" <%if (conjuge != null) { %>
									value="<%=mae%>" <% }%> size="40" maxlength="40"></td>
							</tr>
							<tr>
								<th class="label">Pai</th>
								<td><input type="text" name="pai" <%if (conjuge != null) { %>
									value="<%=pai%>" <% }%> size="40" maxlength="40"></td>
							</tr>
						  <tr>
							...
							<tr>
								...
								<td><input type="text" name="hobby" <%if (hobby != null) { %>
									value="<%=hobby%>" <% }%> size="25" maxlength="25"></td>
							</tr>
							<tr>
								<th class="label">Time</th>
								<td><input type="text" name="time" <%if (time != null) { %>
									value="<%=time%>" <% }%> size="25" maxlength="25"></td>
							</tr>
						</table>
						</div>
						...
						<table>
							<tr>
								<td><br>|<a class="button" href="listarEntidadeDependente.jsp?cdEntidade=<%=cdEntidade%>&acao=listar&selectAba=<%=selectAba%>&acaoAnterior=<%=acao%>">Cadastro Dependente</a>|<br></td>
							</tr>
						</table>
						</div>
