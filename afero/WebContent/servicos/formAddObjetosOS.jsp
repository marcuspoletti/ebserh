...
function confirmaExcluirObj(cont, idObj) {
	  if (confirm("Deseja mesmo apagar o Objeto [" + cont + "]?")) {  
	       location.href = 'formAddObjetosOS.jsp?acao=excObj&idOrdemServico='+document.getElementById('idOrdemServico').value+'&idOrdemServicoObjeto='+idObj+'&idLoja='+document.getElementById('idLoja').value;
	  }
}

function confirmaExcluirItem(cont, idItem) {
	  if (confirm("Deseja mesmo apagar o Serviço [" + cont + "]?")) {  
	       location.href = 'formAddObjetosOS.jsp?acao=excItem&idOrdemServico='+document.getElementById('idOrdemServico').value+'&idOrdemServicoItem='+idItem+'&idLoja='+document.getElementById('idLoja').value;
	  }
}

function confirmaExcluirTarefa(cont, id) {
	  if (confirm("Deseja mesmo apagar a Tarefa [" + cont + "]?")) {  
	       location.href = 'formAddObjetosOS.jsp?acao=excTaf&idOrdemServico='+document.getElementById('idOrdemServico').value+'&idOrdemServicoTarefa='+id+'&idLoja='+document.getElementById('idLoja').value;
	  }
}
function salvar() {
...
<input type="hidden" id="cdEntidade" name="cdEntidade" value="<%=cdEntidade %>"/>
...
<!--  Cabeçalho  -->
<table border="0" align="center" height="20%" width="80%">
	<tr>
...
<%} %>
<p align="left"><a class="link" href="formAddObjetosOS.jsp?acao=<%=acao%>&idOrdemServico=<%=idOrdemServico%>&idLoja=<%=idLoja %>" style="font-size: 14px; float: left;">Atualizar</a>
<br>
<br>
<p align="right"><a class="grid" target="blank" href="comercial/formPedido.jsp?acao=inc&idOrdemServico=<%=idOrdemServico%>&cdEntidade=<%=cdEntidade%>&idEntrega=<%="1"%>"   style="font-size: 14px; float: right;">Adic. Ped. Produto</a>
<br>
<br>
<hr>
<%--<p align="right"><a class="grid" target="blank" href="listarListaTarefa.jsp"   style="font-size: 14px; float: right;"><%="|" +" Cadastrar Tarefas " + " | "%></a> --%>
<p align="right"><a class="grid" target="principal" href="listarOrdemServico.jsp"   style="font-size: 14px; float: right;"><%="|" + " Sair " + " | " %></a>
...
daoObjeto = new OrdemServicoObjetoDAO(conn);
boolean existeServico = daoObjeto.existeObjeto(Integer.parseInt(idOrdemServico));
if(!existeServico){
%>
...
%>
	<tr>
		...
		<td class="grid" width="10%"><%=Utilitaria.formatarNumero(ordemServicoItem.getVlUni(), 2) %></td>
		<td class="grid" width="10%"><%=Utilitaria.formatarNumero(ordemServicoItem.getQuant(), 0) %></td>
		<td class="grid" width="10%"><%=Utilitaria.formatarNumero(ordemServicoItem.getpDesc(), 2) %></td>
		<td class="grid" width="30%"><%=Utilitaria.formatarNumero(ordemServicoItem.getValor(), 2) %></td>
		<td class="grid" width="40%"><a href="javascript:confirmaExcluirItem(<%=cont2%>,<%=ordemServicoItem.getIdOrdemServicoItem() %>);"><img border = "0" src="../images/delete.gif"></a></td>		
	</tr>
<%}%>
<%}%>
</table>
