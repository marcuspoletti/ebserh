...
function initTreeView () { if (self.TVinitd) return;
 ...
 isDHTML = (document.all || document.layers || divs);
 ...
 for (var i = 1; i <= TVcount; i++) {
  ...
  if (isVisible) {
   ...
   if (!TVentries[i][0]) { treePfx = TVentries[i][4];
    ...
    if (document.all) document.all["TV"+i].innerHTML = retVal;
    else if (divs) 
  ...
  if (TVentries[i][7] != isVisible) { TVentries[i][7] = isVisible;
   ...
   else if (document.all) document.all["TV"+i].style.display = (isVisible ? "block" : "none");
 }} // scroll new entry into view
 if (TVelemTop > 0) { TVelemBtm += EntryHeight;
  if (document.layers || (divs && !document.all)) { var ScreenTop = window.pageYOffset;
...
function idx (text, key, link, prefix, code, opts) {
 ...
 if (document.all || divs) retVal = "<DIV ID=\"TV"+ TVcount +"\""
