...
document.write('.btn     { width: 22px; height: 22px; border: 1px solid; margin: 0; padding: 0; }\n');
...
this.btnList = {
    ...
    "strikethrough":  ['StrikeThrough',        'Riscado',            'editor_action(this.id)',  'ed_format_strike.gif'],
    "subscript":      ['SubScript',            'Subscrito',          'editor_action(this.id)',  'ed_format_sub.gif'],
    "superscript":    ['SuperScript',          'Subrescrito',        'editor_action(this.id)',  'ed_format_sup.gif'],
    "justifyleft":    ['JustifyLeft',          'Alinha à esquerda', 'editor_action(this.id)',  'ed_align_left.gif'],
    "justifycenter":  ['JustifyCenter',        'Alinha pelo centro','editor_action(this.id)',  'ed_align_center.gif'],
    "justifyright":   ['JustifyRight',         'Alinha à direita',  'editor_action(this.id)',  'ed_align_right.gif'],
    "justifyfull":    ['JustifyFull',          'Justificado',       'editor_action(this.id)',  'ed_align_justify.gif'],
    "orderedlist":    ['InsertOrderedList',    'Lista com numeração','editor_action(this.id)',  'ed_list_num.gif'],
    "unorderedlist":  ['InsertUnorderedList',  'Lista com marcadores', 'editor_action(this.id)',  'ed_list_bullet.gif'],
    "outdent":        ['Outdent',              'Recua',             'editor_action(this.id)',  'ed_indent_less.gif'],
    "indent":         ['Indent',               'Avança',            'editor_action(this.id)',  'ed_indent_more.gif'],
    ...
    "createlink":     ['CreateLink',           'Insere link',        'editor_action(this.id)',  'ed_link.gif'],
    "insertimage":    ['InsertImage',          'Insere imagem',      'editor_action(this.id)',  'ed_image.gif'],
    "inserttable":    ['InsertTable',          'Insere tabela',      'editor_action(this.id)',  'insert_table.gif'],
    "htmlmode":       ['HtmlMode',             'Ver código HTML',    'editor_setmode(\''+objname+'\')', 'ed_html.gif'],
    "popupeditor":    ['popupeditor',          'Aumentar o editor',  'editor_action(this.id)',  'fullscreen_maximize.gif'],
    "about":          ['about',                'Crédito',  'editor_about(\''+objname+'\')',  'ed_about.gif'],
...
function editor_generate(objname,userConfig) {
  ...
  for (var btnGroup in config.toolbar) {
    ...
    for (var btnItem in config.toolbar[btnGroup]) {
      ...
      toolbar += '<button title="' +btnTitle+ '" id="_' +objname+ '_' +btnCmdID+ '" style="background-color:'+config.bgcolor+'; border-color:'+config.bgcolor+'" class="btn" onClick="' +btnOnClick+ '" onmouseover="if(this.className==\'btn\'){this.className=\'btnOver\'}" onmouseout="if(this.className==\'btnOver\'){this.className=\'btn\'}" unselectable="on"><img src="' +config.imgURL + btnImage+ '" border=0 unselectable="on"></button>';
  ...
  var editor = '<span id="_editor_toolbar"><table border=0 cellspacing=0 cellpadding=0 bgcolor="'+config.bgcolor+'" style="padding: 1 0 0 2" width=' + config.width + ' unselectable="on"><tr><td>\n'
  + toolbar
  + '</td></tr></table>\n'
  + '</td></tr></table></span>\n'
