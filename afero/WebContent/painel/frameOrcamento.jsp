...
clausula = " WHERE o.status = '"+status+"' AND o.dtOrc BETWEEN '"+converte.DMYToYMDI(dataIni)+" 00:00:00' AND '"+converte.DMYToYMDI(dataFim)+" 23:59:59'"; 
    
if(!idLoja.equalsIgnoreCase("0")){
	clausula = clausula + " AND o.idLoja = " + Integer.parseInt(idLoja);
}
...
for ( Iterator it = list.iterator(); it.hasNext(); ) {
	Orcamento orcamento = (Orcamento) it.next();;
	Orcamento orc = dao.procurarOrcamento(orcamento.getIdOrcamento());
	...
	entrega = daoEntrega.procurarEntrega(orc.getIdEntrega());
	...
	}else if(orc.getStatus().equalsIgnoreCase("F")){
		dsStatus="Faturado";
	}else if(orc.getStatus().equalsIgnoreCase("R")){
		dsStatus="Recusado";
	}
    valorTotal = valorTotal + (orc.getVlOrc()+entrega.getTxEntrega());
%>
   ...
   <tr>
      ...
      <%} %>
      <td class = "grid" width="2%"><center><a target="_blank" href="orcamento.jsp?idOrcamento=<%=orc.getIdOrcamento()%>"><%=orc.getIdOrcamento()%></a></center></td>
      <td class = "grid" width="30%"><center><%=entidade.getNome()%></center></td>
      <td class = "grid" width="5%"><center><%=Utilitaria.formatarNumero(orc.getVlOrc()+entrega.getTxEntrega(),2)%></center></td>
