...
function localizar() {
  document.forms[0].action="listaCompras.jsp";
...
function recarregar(acao) {
  document.forms[0].action='listaCompras.jsp?acao='+acao;
  document.forms[0].submit();
}
...
clausula = clausula+" WHERE p.status='A' AND e.qtEstoque <= e.qtMinimo AND e.status= 'A' AND e.idLoja = "+idLoja+"  AND precoPadrao = 'S' " ;


if (nomeProduto!= null) {
	  clausula += " AND p.dsProduto LIKE '"+nomeProduto+"%'";
}else{
	  clausula += " AND p.dsProduto is not null ";
}
if(idDivisao != 0){
	  clausula += " AND p.idDivisao = "+idDivisao;	
}
clausula += " ORDER BY ordem desc ";
...
<h1 class="cabecalho_pagina">Lista de Compras</h1>

<hr>

<body onload="document.forms[0].elements[0].focus();" >
<form action="listaCompras.jsp?acao=listar">
...
for ( Iterator it = list.iterator(); hasNext && cont < gridSize; hasNext = it.hasNext()) {
	...
	listarEstoque = daoEstoque.procurarEstoquePrecoQuantidades(prod.getIdProduto(), idLoja);
...
%>
    <tr>
     ...
     <td class = "grid"><center><%=prod.getDsProduto() %></center></td> 
      ...
      <td class = "grid" width="5%"><center><b><i><%=Utilitaria.formatarNumero((estoque.getQtMaximo()-estoque.getQtEstoque()),2)%></i></b></center></td>
...
<%  if (rowNum != 0 || hasNext) { %>
        <center>
<%      if (rowNum != 0) { %>
<a class="link" href="listaCompras.jsp?rowNum=<%=0%>&nomeProduto=<%=nomeProduto%>&status=<%=status%>&idLoja=<%=idLoja%>&idDivisao=<%=idDivisao%>">Primeira</a>&nbsp&nbsp
|&nbsp&nbsp<a class="link" href="listaCompras.jsp?rowNum=<%=rowNum-gridSize%>&nomeProduto=<%=nomeProduto%>&status=<%=status%>&idLoja=<%=idLoja%>&idDivisao=<%=idDivisao%>">< Anterior</a>&nbsp&nbsp
<%      } else { %>
Primeira&nbsp&nbsp|&nbsp&nbsp< Anterior&nbsp
<%      } %>
<%      if (hasNext) { %>
|&nbsp&nbsp<a class="link" href="listaCompras.jsp?rowNum=<%=rowNum+gridSize%>&nomeProduto=<%=nomeProduto%>&status=<%=status%>&idLoja=<%=idLoja%>&idDivisao=<%=idDivisao%>">Próxima ></a>&nbsp&nbsp
|&nbsp&nbsp<a class="link" href="listaCompras.jsp?rowNum=<%=rowCount-gridSize%>&nomeProduto=<%=nomeProduto%>&status=<%=status%>&idLoja=<%=idLoja%>&idDivisao=<%=idDivisao%>">Última</a>
<%      } else { %>
