...
clausula = " WHERE ps.status = '"+status+"' AND ps.dtPed BETWEEN '"+converte.DMYToYMDI(dataIni)+" 00:00:00' AND '"+converte.DMYToYMDI(dataFim)+" 23:59:59'"; 
...
list = dao.listarPedidoSaidaTipoPedido(clausula);
...
<table border="0" width="100%">
    <tr>
      ...
      <th class="grid" width="2%">Tipo Pedido</th>
    </tr>
<%
int numOrcamento = 0;
String dsStatus = "";
...
for ( Iterator it = list.iterator(); it.hasNext(); ) {
	...
	PedidoSaida pedSaida = dao.procurarPedidoSaidaItemTipoPedido(pedidoSaida.getIdPedidoSaida());
	...
	if(pedSaida.getTipoPedido() != null){
		if(pedSaida.getTipoPedido().equalsIgnoreCase("M")){
			tipoPedido = "MESA";
		}else if(pedSaida.getTipoPedido().equalsIgnoreCase("P")){
			tipoPedido = "PRESENCIAL";
		}else if(pedSaida.getTipoPedido().equalsIgnoreCase("T")){
			tipoPedido = "TELEFONISTA";
		}else if(pedSaida.getTipoPedido().equalsIgnoreCase("R")){
			tipoPedido = "RAPIDO";
		}else if(pedSaida.getTipoPedido().equalsIgnoreCase("PZ")){
			tipoPedido = "PIZZARIA";
		}else{
			tipoPedido = "";
		}
    ...
    valorTotal += pedSaida.getVlPed()+entrega.getTxEntrega();
    total = pedSaida.getVlPed();
%>
   ...
   <tr>
      ...
      <td class = "grid" width="5%"><center><%=Utilitaria.formatarNumero(total + entrega.getTxEntrega(),2)%></center></td>
      <td class = "grid" width="3%"><font color="red"><center><%=dsStatus%></center></font></td>
      <td class = "grid" width="3%"><font color="red"><center><%=tipoPedido%></center></font></td>
  	</tr>
<%
}
%>      
