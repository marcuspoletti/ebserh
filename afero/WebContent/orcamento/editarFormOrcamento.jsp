...
if(acao.equalsIgnoreCase("atu")){
	...
	Orcamento orc = daoOrcamentoUpdate.procurarOrcamento(Integer.parseInt(idOrcamento));
	...
	int cdEntidadeUpdate = 0;
	if(request.getParameter("cdEntidade") == null){
		cdEntidadeUpdate = orc.getCdEntidade();
	}else{
		cdEntidadeUpdate =  Utilitaria.toNumber(request.getParameter("cdEntidade")).intValue();
	}
	...
	hrEntrega = daoOrcamentoUpdate.procurarHrEntrega(Integer.parseInt(idOrcamento));
	if(request.getParameter("hrEntrega") == null){
		hrEntrega = orc.getHrEntrega();
	}else{
		hrEntrega = "07:00";
	}
...
<h1 class="cabecalho_pagina">Editar Orçamento</h1>
  ...
  <tr>
    ...
    <tr>
      <th class="label" style="height: 22px">Cliente*</th>
            <td style="height: 15px"><input disabled type="text" name="dsEntidade" <%if (dsEntidade != null) { %>value="<%=dsEntidade%>"<% }%>  size="60" maxlength="60"><a href="editarConsultarClientes.jsp?idOrcamento=<%=idOrcamento%>&acao=<%=acao%>&idLoja=<%=idLoja%>&idEntrega=<%=idEntrega%>&idColaborador=<%=idColaborador%>"><img border = "0" src="../images/pesquisa.gif"></a> </td>
