...
PedidoSaidaDAO daoPedidoSaidaExistente = new PedidoSaidaDAO(conn);  
...
String hrEntrega = daoOrcamento.procurarHrEntrega(Integer.parseInt(idOrcamento));
if(hrEntrega == null){
	hrEntrega = "07:00";
}
...
if(acao.equalsIgnoreCase("gravar") ){
	 if(status.equals("A") && !ok){
		...
		OrcamentoDAO daoOrcamentoUpdate = new OrcamentoDAO(conn);
		...
		OrcamentoItem orcamentoItemUpdate = null;
		OrcamentoItemDAO daoOrcamentoItemUpdate = new OrcamentoItemDAO(conn);
		...
		int id = daoPedidoSaida.idPedidoSaida();
		daoPedidoSaida.atualizarTipoPedido(id, pedidoSaida.getTipoPedido());
		daoPedidoSaida.atualizarStatusPag(id, pedidoSaida.getStatusPag());
		...
		for ( Iterator it = listUpdate.iterator(); it.hasNext(); ) {
			orcamentoItemUpdate = (OrcamentoItem) it.next();
			...
			PedidoSaidaItemDAO daoPedidoSaidaItem = new PedidoSaidaItemDAO(conn);
			daoPedidoSaidaItem.incluir(pedidoSaidaItem);
		}
		
	}else if((status.equals("R") || status.equals("NA"))){
		OrcamentoDAO daoOrcamentoUp = new OrcamentoDAO(conn);
		Orcamento orcUpdate = daoOrcamentoUp.procurarOrcamento(Integer.parseInt(idOrcamento));
...
<table border="0" width="100%">
  ...
  <tr>
   ...
   <tr>
    ...
    <td><input disabled type="text" name="hrEntrega" <%if (hrEntrega != null) { %>value="<%=hrEntrega%>"<% }%>  size="30" maxlength="30"></td>
  </tr>
   <tr>
