...
%>
    <tr>
      ...
      <td class = "grid" width="30%"><center><a class="link" target="principal" href="formPedido.jsp?cdEntidade=<%=cdEntidade%>&acao=<%=acao%>&dsEntidade=<%=nomeEntidade%>&idLoja=<%=idLoja%>&idEntrega=<%=idEntrega%>&idColaborador=<%=idColaborador%>"onclick="javascript:window.close()"><%=nomeEntidade%></a></center></td>
...
<%  if (rowNum != 0 || hasNext) { %>
        <center>
<%      if (rowNum != 0) { %>
<a class="link" href="consultarClientes.jsp?rowNum=<%=0%>&nomeLocalizar=<%=nomeLocalizar%>&statusLocalizar=<%=statusLocalizar%>&tpInsc=<%=tpInsc%>&cpfLocalizar=<%=cpfLocalizar%>&telefone=<%=telefone%>&dsEndereco=<%=dsEndereco%>&cdTipoLogradouro=<%=cdTipoLogradouro%>">Primeira</a>&nbsp&nbsp
|&nbsp&nbsp<a class="link" href="consultarClientes.jsp?rowNum=<%=rowNum-gridSize%>&nomeLocalizar=<%=nomeLocalizar%>&statusLocalizar=<%=statusLocalizar%>&tpInsc=<%=tpInsc%>&cpfLocalizar=<%=cpfLocalizar%>&telefone=<%=telefone%>&dsEndereco=<%=dsEndereco%>&cdTipoLogradouro=<%=cdTipoLogradouro%>">< Anterior</a>&nbsp&nbsp
<%      } else { %>
Primeira&nbsp&nbsp|&nbsp&nbsp< Anterior&nbsp
<%      } %>
<%      if (hasNext) { %>
|&nbsp&nbsp<a class="link" href="consultarClientes.jsp?rowNum=<%=rowNum+gridSize%>&nomeLocalizar=<%=nomeLocalizar%>&statusLocalizar=<%=statusLocalizar%>&tpInsc=<%=tpInsc%>&cpfLocalizar=<%=cpfLocalizar%>&telefone=<%=telefone%>&dsEndereco=<%=dsEndereco%>&cdTipoLogradouro=<%=cdTipoLogradouro%>">Próxima ></a>&nbsp&nbsp
|&nbsp&nbsp<a class="link" href="consultarClientes.jsp?rowNum=<%=rowCount-gridSize%>&nomeLocalizar=<%=nomeLocalizar%>&statusLocalizar=<%=statusLocalizar%>&tpInsc=<%=tpInsc%>&cpfLocalizar=<%=cpfLocalizar%>&telefone=<%=telefone%>&dsEndereco=<%=dsEndereco%>&cdTipoLogradouro=<%=cdTipoLogradouro%>">Última</a>
<%      } else { %>
