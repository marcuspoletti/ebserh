...
function cancelar() {
  document.forms[0].action="formTipoRaca.jsp";
  document.forms[0].submit();
}

function voltar() {
  document.forms[0].action = 'listarTipoRaca.jsp?acao=voltar'
	document.forms[0].submit();
}  
</script>
</head>
...
if (acao.equalsIgnoreCase("atu")) {
  TipoRacaDAO dao = new TipoRacaDAO(conn);
  TipoRaca tipoRaca = dao.procurarTipoRaca(Integer.parseInt(idTipoRaca));
	descricao  = tipoRaca.getDsTipoRaca();
	status = tipoRaca.getStatus();
}
%>
<body onload="document.forms[0].elements[2].focus();" >
<h1 class="cabecalho_pagina">Cadastro de Tipo de Raça</h1>
<form method="post" action="listarTipoRaca.jsp?acao=<%=acao%>&idTipoRaca=<%=idTipoRaca%>">
<input type="hidden" name="acao" value="<%=acao%>"/>
<input type="hidden" name="idTipoRaca" value="<%=idTipoRaca%>"/>
