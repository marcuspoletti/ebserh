...
function atualizar() {
 document.forms[0].action='apagarRegistrosPorPedidoSaida.jsp';
...
if(acao.equals("ok")){
	...
	String clausula = " WHERE (ps.integracao not in ('S') OR ps.integracao is null)AND ps.status in ('P', 'C', 'A') AND ps.operacao in ('S', 'T', 'ES', 'AS') AND ps.idPedidoSaida="+idPedidoLocalizar+" AND ps.idLoja ="+idLoja + " AND ps.idPedidoSaida not in (Select idPedidoSaida from tbretornonotafiscal) ";
...
<form method="get" action="apagarRegistrosPorPedidoSaida.jsp">
<table colspan="2">
<tr>
