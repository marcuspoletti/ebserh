...
function atualizar() {
 document.forms[0].action='apagarRegistrosSaidasPorLoja.jsp';
...
if(acao.equals("ok")){
	...
	PedidoSaidaDAO dao = new PedidoSaidaDAO(conn);
	List listOrc;
	PedidoSaidaSubItemDAO daoPedidoSaidaItem = new PedidoSaidaSubItemDAO(conn); 
	String clausula = " WHERE (ps.integracao not in ('S') OR ps.integracao is null) AND ps.status in ('P', 'C', 'A', 'F') AND ps.operacao in ('S', 'T', 'ES', 'AS') AND ps.dtEntrega BETWEEN '"+converte.DMYToYMDI(dataIni)+" 00:00:00' AND '"+converte.DMYToYMDI(dataFim)+" 23:59:59' AND ps.idLoja ="+idLoja +" AND ps.idPedidoSaida not in (Select idPedidoSaida from tbretornonotafiscal) ";
...
<h1 class="cabecalho_pagina">Apagar Registros de Saídas Por Loja</h1>

<hr>

<body onload="document.forms[0].elements[0].focus();" >
<form method="get" action="apagarRegistrosSaidasPorLoja.jsp">
...
<table colspan="2">
  ...
  <table border="0" width="100%">
    <tr>
      <td class='grid'>Data de Entrega Ini.(a partir de:)
	<input type="text" name="dataIni" size="15" value="<%= dataIni %>" onblur="chkData(this,'dd/MM/yyyy')" maxlength="10" class="inputs"><a href="javascript:void(0)" onclick="if(self.gfPop)gfPop.fPopCalendar(document.forms[0].dataIni);return false;" HIDEFOCUS><img class="PopcalTrigger" align="absmiddle" src="../js/calendar/calbtn.gif" width="34" height="22" border="0" alt=""></a>
      ...
      <td class='grid'>Data de Entrega Final(até a:) 
